...
class TestItemizedResource(unittest.TestCase):
    ...
    def test_well_get_absolute_location(self):
        self.assertEqual(
            self.plate.get_item(0).get_absolute_location(), Coordinate(0, 63, 0)
        )
        self.assertEqual(
            self.plate.get_item(7).get_absolute_location(), Coordinate(0, 0, 0)
        )
        self.assertEqual(
            self.plate.get_item(8).get_absolute_location(), Coordinate(9, 63, 0)
        )
        self.assertEqual(
            self.plate.get_item(17).get_absolute_location(), Coordinate(18, 54, 0)
        )
        self.assertEqual(
            self.plate.get_item(95).get_absolute_location(), Coordinate(99, 0, 0)
        )
    ...
    def _traverse_test(
        ...
        items: List[Well] = []
        for wells in self.plate.traverse(
            batch_size=2, direction=direction, repeat=False
        ):
