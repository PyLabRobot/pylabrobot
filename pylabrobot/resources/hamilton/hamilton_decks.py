...
class HamiltonDeck(Deck, metaclass=ABCMeta):
    ...
    def assign_child_resource(
        ...
        if self.has_resource(resource.name):
            if replace:
                ...
                raise ValueError(
                    f"Resource with name '{resource.name}' already defined."
                )
    ...
    def summary(self) -> str:
        ...
        def find_longest_child_name(resource: Resource, depth=0, depth_weight=4):
            ...
            new_depth = (
                depth + 1 if resource.category not in exclude_categories else depth
            )
            return max(
                [(l + d * depth_weight)]
        ...
        def find_longest_type_name(resource: Resource):
            ...
            return max(
                [l] + [find_longest_type_name(child) for child in resource.children]
            )
        ...
        sorted_resources = sorted(
            self.children, key=lambda r: r.get_absolute_location().x
        )
...
class HamiltonSTARDeck(HamiltonDeck):  # pylint: disable=invalid-name
    ...
    def __init__(
        ...
        if not no_trash:
            ...
            self.assign_child_resource(
                resource=self._trash96, location=Coordinate(x=-232.1, y=110.3, z=189.0)
            )  # 165.0 -> 189.0
