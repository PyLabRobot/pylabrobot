...
class Carrier(Resource, Generic[S]):
    ...
    def unassign_child_resource(self, resource):
        ...
        if (
            not isinstance(resource.parent, CarrierSite)
            or not resource.parent.parent == self
        ):
...
class PlateCarrierSite(CarrierSite):
    ...
    def _deregister_resource_stack_callback(self, resource: Resource):
        ...
        ):  # the ResourceStack itself is unassigned
...
def create_carrier_sites(
    ...
    **kwargs,
    ...
    for spot, (l, x, y) in enumerate(zip(locations, site_size_x, site_size_y)):
        site = klass(
            name=f"carrier-site-{spot}", size_x=x, size_y=y, size_z=0, **kwargs
        )
        site.location = l
        sites.append(site)
    return sites
