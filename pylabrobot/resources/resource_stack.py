...
class ResourceStack(ResourceHolderMixin, Resource):
    ...
    def get_default_child_location(self, resource: Resource) -> Coordinate:
        return (
            super().get_default_child_location(resource)
            + self.get_resource_stack_edge()
        )
    ...
    def assign_child_resource(
        ...
        if len(self.children) > 0:
            ...
            if isinstance(resource, Lid) and isinstance(top_item, Plate):
                logger.warning(
                    "Assigning a lid to a resource stack is deprecated and will be "
                ...
                top_item.assign_child_resource(
                    resource, location=get_child_location(resource) + resource_location
                )
                return
    ...
    def unassign_child_resource(self, resource: Resource):
        if (
            self.direction == "z" and resource != self.children[-1]
        ):  # no floating resources
            raise ValueError(
