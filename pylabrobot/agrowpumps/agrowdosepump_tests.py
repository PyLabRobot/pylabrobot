...
class TestAgrowPumps(unittest.IsolatedAsyncioTestCase):
  async def asyncSetUp(self):
    ...
    async def _mock_setup_modbus():
      self.device._agrow_driver._modbus = SimulatedModbusClient()

    self.device._agrow_driver._setup_modbus = _mock_setup_modbus
  ...
  async def test_setup(self):
    self.assertEqual(self.device._agrow_driver.port, "simulated")
    self.assertEqual(self.device._agrow_driver.address, 1)
    self.assertEqual(len(self.device.pumps), 6)
    self.assertEqual(
      self.device._agrow_driver._pump_index_to_address,
      {pump: pump + 100 for pump in range(0, 6)},
    )

  async def test_run_continuously(self):
    self.device._agrow_driver.modbus.write_register.reset_mock()
    await self.device.pumps[0].run_continuously(speed=1)
    self.device._agrow_driver.modbus.write_register.assert_called_once_with(100, 1, unit=1)
  ...
  async def test_halt_single_channel(self):
    ...
    self.device._agrow_driver.modbus.write_register.assert_called_once_with(102, 0, unit=1)
