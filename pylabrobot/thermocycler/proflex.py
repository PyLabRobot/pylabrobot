...
class PCRProtocol:
    ...
    def add_step(self, temp_list: list[float], time: int = 0, ramprate: int = 100, hold: bool = True,
        ...
        step['multiline'].append({
            'cmd': 'RAMP',
            'params': {'rate': str(ramprate)},
            'args': [str(t) for t in temp_list]
        })
    ...
    def generate_run_info_files(
        ...
        pretty_xml_as_string = xml_declaration + reparsed.toprettyxml(indent="   ")[len('<?xml version="1.0" ?>'):]
...
class Proflex(thermocyclerBackend):
    ...
    async def _scpi_authenticate(self):
        ...
        acc_res = await self.scpi_send_data({"cmd": "ACCess", "params": {'stealth': True}, "args": ["Controller"]})
    ...
    async def scpi_check_run_exists(self, runName="testrun"):
        res = await self.scpi_send_data({"cmd": f"RUNS:EXISTS?", "args": [runName], "params": {"type": "folders"}})
        if self._parse_scpi_response(res)["status"] != "OK":
    ...
    async def scpi_create_run(self, runName="testrun"):
        res = await self.scpi_send_data({"cmd": f"RUNS:NEW", "args": [runName]}, response_timeout=10)
        if self._parse_scpi_response(res)["status"] != "OK":
    ...
    async def _scpi_run_protocol(self, protocol: PCRProtocol, runName="testrun", user="Guest"):
        load_res = await self.scpi_send_data(protocol.gen_protocol_data(), response_timeout=5, readonce=False)
        ...
        }, response_timeout=2, readonce=False)
    ...
    async def check_if_running(self, protocol: PCRProtocol):
        ...
        if progress["RunTitle"] == "-":
            res = await self._read_response(timeout=5)
            logging.info("Protocol completed")
            return False, 'completed', self.prot_time_elapsed, 0
        ...
        if progress['Stage'] != "-" and progress['Step'] != "-":
            if [int(progress['Stage']), int(progress['Step'])] in protocol.infinite_holds:
                while True:
                    ...
                    if all(abs(float(block_temps[i]) - target_temps[i]) < 0.5 for i in range(len(block_temps))):
                        break
