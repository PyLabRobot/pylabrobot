...
class HighResSampleStorageAutomatedRetrievalBackend(AutomatedRetrievalBackend):
  ...
  *nests* (transfer stations); the internal spatula moves plates between a nest
  ...
  async def request_axis_positions(self) -> Dict[str, float]:
    ...
    out: Dict[str, float] = {}
    for key, value in _parse_kv(await self._driver.send_command("status")).items():
      try:
        out[key] = float(value)
      except ValueError:
        continue
  ...
  async def request_door_status(self) -> Dict[str, DoorState]:
    ...
    for name, value in _parse_kv(await self._driver.send_command("doorstatus")).items():
      state = value.lower()
  ...
  async def request_nest_status(self) -> Dict[int, NestState]:
    ...
    for key, value in _parse_kv(await self._driver.send_command("neststatus")).items():
      try:
        nest = int(key)
      except ValueError:
        continue
...
class HighResSampleStorageDriver(Driver):
  ...
  async def request_version(self) -> VersionInfo:
    raw = _parse_kv(await self.send_command("version"))
    return VersionInfo(
