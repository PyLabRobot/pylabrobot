...
class TestKeyenceSendCommand(unittest.IsolatedAsyncioTestCase):
  ...
  async def test_stops_reading_at_terminator_multibyte_chunk(self):
    # Multi-byte chunks ending in \r must break immediately without waiting for timeout.
    self.backend.io.read.side_effect = [b"MOTORON\r"]  # type: ignore
    response = await self.backend.send_command("RMOTOR")
    self.assertEqual(response, "MOTORON")
    self.assertEqual(self.backend.io.read.await_count, 1)  # type: ignore
