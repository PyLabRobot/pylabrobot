...
class HamiltonHeatShaker(HeaterShakerBackend, USBBackend):
    ...
    def __init__(
        self, shaker_index: int, id_vendor: int = 0x8AF, id_product: int = 0x8002
    ) -> None:
        """
    ...
    async def shake(
        ...
        assert (
            500 <= acceleration <= 10_000
        ), "Acceleration must be between 500 and 10_000"

        await self._start_shaking(
            direction=direction, speed=int_speed, acceleration=acceleration
        )
