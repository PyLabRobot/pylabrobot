"""Legacy. Use pylabrobot.inheco.scila.SCILABackend instead."""
...
class SCILABackend(MachineBackend):
  ...
  def __init__(self, scila_ip: str, client_ip: Optional[str] = None) -> None:
    self._new = new_scila.SCILABackend(scila_ip=scila_ip, client_ip=client_ip)

  @property
  def _sila_interface(self):
    return self._new._sila_interface
  ...
  async def request_status(self) -> str:
    return await self._new.request_status()

  async def request_liquid_level(self) -> str:
    return await self._new.request_liquid_level()

  async def request_temperature_information(self) -> dict[str, Any]:
    return await self._new.request_temperature_information()
  ...
  async def is_temperature_control_enabled(self) -> bool:
    return await self._new.is_temperature_control_enabled()

  async def open(self, drawer_id: int) -> None:
    await self._new.open(drawer_id=drawer_id)

  async def close(self, drawer_id: int) -> None:
    await self._new.close(drawer_id=drawer_id)

  async def request_drawer_statuses(self) -> Dict[int, DrawerStatus]:
    return await self._new.request_drawer_statuses()

  async def request_drawer_status(self, drawer_id: int) -> DrawerStatus:
    return await self._new.request_drawer_status(drawer_id=drawer_id)

  async def request_co2_flow_status(self) -> str:
    return await self._new.request_co2_flow_status()

  async def request_valve_status(self) -> dict[str, str]:
    return await self._new.request_valve_status()
