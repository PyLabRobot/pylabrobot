...
class SCILABackend(MachineBackend):
  ...
  def __init__(self, scila_ip: str, client_ip: Optional[str] = None) -> None:
    self._driver = SCILADriver(scila_ip=scila_ip, client_ip=client_ip)
    self._temp = SCILATemperatureBackend(driver=self._driver)

  @property
  def _sila_interface(self):
    return self._driver._sila_interface
  ...
  async def request_liquid_level(self) -> str:
    return await self._driver.request_liquid_level()
  ...
  async def open(self, drawer_id: int) -> None:
    await self._driver.open(drawer_id=drawer_id)

  async def close(self, drawer_id: int) -> None:
    await self._driver.close(drawer_id=drawer_id)

  async def request_drawer_statuses(self) -> Dict[int, DrawerStatus]:
    return await self._driver.request_drawer_statuses()

  async def request_drawer_status(self, drawer_id: int) -> DrawerStatus:
    return await self._driver.request_drawer_status(drawer_id=drawer_id)

  async def request_co2_flow_status(self) -> str:
    return await self._driver.request_co2_flow_status()

  async def request_valve_status(self) -> dict[str, str]:
    return await self._driver.request_valve_status()
