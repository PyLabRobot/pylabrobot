...
class XPeelBackend(PeelerBackend):
  ...
  def __init__(self, port: str, timeout=None):
    self.driver = XPeelDriver(port=port, timeout=timeout)
    self._peeler = XPeelPeelerBackend(self.driver)
  ...
  async def get_version(self):
    return await self.driver.request_version()

  async def seal_check(self):
    return await self.driver.seal_check()

  async def get_tape_remaining(self):
    return await self.driver.request_tape_remaining()

  async def enable_plate_check(self, enabled=True):
    return await self.driver.enable_plate_check(enabled=enabled)

  async def get_seal_sensor_status(self):
    return await self.driver.request_seal_sensor_status()

  async def set_seal_threshold_upper(self, value: int):
    return await self.driver.set_seal_threshold_upper(value=value)

  async def set_seal_threshold_lower(self, value: int):
    return await self.driver.set_seal_threshold_lower(value=value)

  async def move_conveyor_out(self):
    return await self.driver.move_conveyor_out()

  async def move_conveyor_in(self):
    return await self.driver.move_conveyor_in()

  async def move_elevator_down(self):
    return await self.driver.move_elevator_down()

  async def move_elevator_up(self):
    return await self.driver.move_elevator_up()

  async def advance_tape(self):
    return await self.driver.advance_tape()
