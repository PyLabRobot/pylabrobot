"""Offline robot-server transport and capture replay for Opentrons devices."""
...
class ChatterboxHTTP(HTTP):
  ...
  and pass it as the ``io`` of an ``OT2`` or ``Flex`` to drive the device
  ...
  def __init__(
    ...
    *,
    ...
    robot_model: str = "OT-3 Standard",
  ) -> None:
    """Configure the offline robot and optional failure responses.
    ...
    Args:
      ...
      robot_model: Model reported by health, typically OT-2 Standard or OT-3 Standard.
    """
  ...
  def _get(self, path: str) -> Dict[str, Any]:
    ...
    if path == "/pipettes":
      mounted = {mount: {"name": name} for name, *_specs, mount in self._pipettes}
      return {mount: mounted.get(mount, {"name": None}) for mount in ("left", "right")}
    if path == "/modules":
      return {"data": []}
    if path == "/runs":
      return {"data": [{"id": "chatterbox-run", "status": self._run_status}]}
    if path == "/instruments":
  ...
  def _post(self, path: str, json: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    if path == "/runs/chatterbox-run/actions":
    ...
    if path.endswith("/commands"):
      ...
      if ctype == "loadPipette":
        ...
        labware_id = params.get("labwareId") or f"chatterbox-labware-{self._labware_load_count}"
