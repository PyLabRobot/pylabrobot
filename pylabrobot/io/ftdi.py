...
class FTDI(IOBase):
  ...
  async def setup(self):
    ...
    try:
      ...
      if self._handshake_fn:
        logger.debug("Performing device handshake...")
        if not await self._handshake_fn(self):
          self._dev.close()
          raise RuntimeError(
            f"Device handshake failed for {self._resolved_serial}. "
            "Device may not be the expected model or is malfunctioning."
          )
