...
try:
    from pylibftdi import Device, FtdiError, LibraryMissingError, Driver
    HAS_PYLIBFTDI = True
...
class FTDI(IOBase):
    ...
    def __init__(
        ...
        if not device_id and not (vid and pid):
            raise ValueError(
                "Must specify either device_id or both vid and pid."
            )
    ...
    def _resolve_device_serial(self) -> str:
        ...
        if self._device_id:
            ...
            if self._vid is not None and self._pid is not None:
                ...
                logger.info(
                    f"Using device_id without VID:PID verification (not provided): "
                    f"{self._device_id}"
                )
        ...
        # Score all devices
        scored_devices = [
            (self._score_device(dev), dev) for dev in usb_devices
        ]
    ...
    async def set_baudrate(self, baudrate: int):
        ...
        capturer.record(FTDICommand(device_id=self.device_id, action="set_baudrate", data=str(baudrate)))
