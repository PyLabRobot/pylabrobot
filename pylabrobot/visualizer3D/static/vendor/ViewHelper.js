...
class ViewHelper extends Object3D {
	...
	constructor( camera, domElement ) {
		...
		/**
		 * LOCAL CHANGE (not upstream): which corner the helper draws in, "left" or "right".
		 * Upstream hard-codes the bottom right, which is where this viewer keeps its zoom controls
		 * and scale bar. Both the draw and the hit test read this, so they cannot drift apart.
		 */
		this.corner = 'right';
		...
		this.render = function ( renderer ) {
			...
			const x = this.corner === 'left' ? 0 : domElement.offsetWidth - dim;
		...
		this.handleClick = function ( event ) {
			...
			// LOCAL CHANGE: corner-aware, and measured against `dim` rather than the distance to the
			// element's far edge, which is only the same number when the helper is right-aligned.
			const offsetX = rect.left + ( this.corner === 'left' ? 0 : domElement.offsetWidth - dim );
