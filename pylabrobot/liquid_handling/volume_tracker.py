...
class VolumeTracker(ABC):
  ...
  @abstractmethod
  def handle(self, op: LiquidHandlingOp) -> None:
    """ Update the pending state with the operation. """

  def queue_op(self, op: LiquidHandlingOp) -> None:
...
class TipVolumeTracker(VolumeTracker):
  ...
  def handle(self, op: LiquidHandlingOp):
    """ Validate a volume operation.

    Args:
      op: The volume operation to handle.

    Raises:
      TipTooLittleLiquidError: If the tip has too little liquid to perform the operation.

      TipTooLittleVolumeError: If the tip has too little volume to perform the operation.
    """
...
class WellVolumeTracker(VolumeTracker):
  ...
  def handle(self, op: LiquidHandlingOp):
    ...
    Raises:
      WellTooLittleLiquidError: If the well has too little liquid to perform the operation.

      WellTooLittleVolumeError: If the well has too little volume to perform the operation.
    """
