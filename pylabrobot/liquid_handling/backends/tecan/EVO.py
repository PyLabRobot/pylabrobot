...
class TecanLiquidHandler(LiquidHandlerBackend, USBBackend, metaclass=ABCMeta):
    ...
    def _assemble_command(
        ...
        cmd = (
            module + command + ",".join(str(a) if a is not None else "" for a in params)
        )
        return f"\02{cmd}\00"
...
class EVO(TecanLiquidHandler):
    ...
    async def setup(self):
        ...
        if (
            self.roma_connected
        ):  # position_initialization_x in reverse order from setup_arm
        ...
        await self.liha.position_absolute_all_axis(
            45, 1031, 90, [1200] * self.num_channels
        )
    ...
    async def aspirate(
        ...
        if any(
            tlc.aspirate_lld if tlc is not None else None
            for tlc in tecan_liquid_classes
        ):
            ...
            await self.liha.move_detect_liquid(
                self._bin_use_channels(use_channels), zadd
            )
            await self.liha.set_z_travel_height([self._z_range] * self.num_channels)
    ...
    async def dispense(self, ops: List[Dispense], use_channels: List[int]):
        ...
        sep, spp, stz, mtr = self._dispense_action(
            ops, use_channels, tecan_liquid_classes
        )
        await self.liha.set_end_speed_plunger(sep)
    ...
    async def pick_up_tips(self, ops: List[Pickup], use_channels: List[int]):
        ...
        await self.liha.get_disposable_tip(
            self._bin_use_channels(use_channels), 768, 210
        )
        # TODO: check z params
    ...
    async def move_resource(self, move: Move):
        ...
        await self.roma.set_vector_coordinate_position(
            1, x, y, z["end"], 900, None, 1, 1
        )
        await self.roma.set_vector_coordinate_position(
            2, x, y, z["travel"], 900, None, 1, 2
        )
        await self.roma.set_vector_coordinate_position(
            3, x, y, z["safe"], 900, None, 1, 3
        )
        await self.roma.set_vector_coordinate_position(
    ...
    def _first_valid(self, l: List[Optional[T]]) -> Tuple[Optional[T], int]:
    ...
    def _liha_positions(
        ...
        List[Optional[int]], List[Optional[int]], Dict[str, List[Optional[int]]]
    ]:
        ...
        def get_z_position(z, z_off, tip_length):
            return int(
                self._z_range - z + z_off * 10 + tip_length
            )  # TODO: verify z formula

        for i, channel in enumerate(use_channels):
            location = (
                ops[i].resource.get_absolute_location() + ops[i].resource.center()
            )
    ...
    def _aspirate_airgap(
        ...
        for i, channel in enumerate(use_channels):
            ...
            if airgap == "lag":
                sep[channel] = int(
                    tlc.aspirate_lag_speed * 12
                )  # 6? TODO: verify step unit
...
class EVOArm:
    ...
    async def report_x_param(self, param: int) -> int:
        ...
        resp: List[int] = (
            await self.backend.send_command(
                module=self.module, command="RPX", params=[param]
            )
        )["data"]
        return resp[0]
    ...
    async def report_y_param(self, param: int) -> List[int]:
        ...
        resp: List[int] = (
            await self.backend.send_command(
                module=self.module, command="RPY", params=[param]
            )
        )["data"]
        return resp
...
class LiHa(EVOArm):
    ...
    async def initialize_plunger(self, tips):
        ...
        """
        await self.backend.send_command(
            module=self.module, command="PID", params=[tips]
        )
    ...
    async def report_z_param(self, param: int) -> List[int]:
        ...
        resp: List[int] = (
            await self.backend.send_command(
                module=self.module, command="RPZ", params=[param]
            )
        )["data"]
        return resp
    ...
    async def report_number_tips(self) -> int:
        ...
        resp: List[int] = (
            await self.backend.send_command(
                module=self.module, command="RNT", params=[1]
            )
        )["data"]
        return resp[0]
    ...
    async def move_detect_liquid(self, channels: int, zadd: List[Optional[int]]):
        ...
        await self.backend.send_command(
            module=self.module, command="MDT", params=[channels] + [None] * 3 + zadd
        )
    ...
    async def get_disposable_tip(self, tips, z_start, z_search):
        ...
        await self.backend.send_command(
            module=self.module, command="AGT", params=[tips, z_start, z_search, 0]
        )
    ...
    async def discard_disposable_tip(self, tips):
        ...
        await self.backend.send_command(
            module=self.module, command="ADT", params=[tips]
        )
...
class RoMa(EVOArm):
    ...
    async def report_r_param(self, param: int) -> int:
        ...
        resp: List[int] = (
            await self.backend.send_command(
                module=self.module, command="RPR", params=[param]
            )
        )["data"]
        return resp[0]
    ...
    async def report_g_param(self, param: int) -> int:
        ...
        resp: List[int] = (
            await self.backend.send_command(
                module=self.module, command="RPG", params=[param]
            )
        )["data"]
        return resp[0]
    ...
    async def set_smooth_move_x(self, mode: int):
        ...
        await self.backend.send_command(
            module=self.module, command="SSM", params=[mode]
        )
    ...
    async def set_vector_coordinate_position(
        ...
        await self.backend.send_command(
            module=self.module, command="SAA", params=[v, x, y, z, r, g, speed, 0, tw]
        )
