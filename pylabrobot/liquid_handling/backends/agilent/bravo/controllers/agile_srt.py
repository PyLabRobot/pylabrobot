...
_SRT_W_PUMP_BLOCK = (
  ("reg", 0x39, "7e9000000d1000"),
  ("reg", 0x3A, "695000000c1000"),
  ("reg", 0x75, "70000000001000"),
  ("reg", 0x76, "90000000001000"),
  ("reg", 0x7C, "40000000fe1000"),
  ("reg", 0x77, "40000000031000"),
  ("reg", 0x78, "70000000001000"),
  ("reg", 0x79, "90000000001000"),
  ("reg", 0x7D, "40000000fe1000"),
  ("reg", 0x7A, "40000000031000"),
  ("op", 0x55),
  ("reg", 0x44, "00000000001000"),
  ("reg", 0xD8, "4b000000071000"),
  ("reg", 0xDA, "40000000001000"),
  ("reg", 0xDE, "64000000061000"),
  ("reg", 0xE2, "00000000001000"),
  ("reg", 0x1F, "00000000001000"),
  ("op", 0x54),
  ("reg", 0x23, "64000000081000"),
  ("reg", 0x04, "64000000061000"),
  ("reg", 0x03, "66666666fe1000"),
  ("reg", 0x02, "77777d0fff1000"),
)
...
class AgileSrtController(Agile7612Controller):
  ...
  def open_serial(self, port: str) -> None:
    raise BravoError(
      ErrorType.COULD_NOT_CONNECT, custom_text="Bravo SRT does not support serial; use Ethernet"
    )
  ...
  def home_axes(self, axes: list[Axis]) -> None:
    ...
    if unsupported:
      raise BravoError(
        ...
        custom_text=(f"Bravo SRT has no {', '.join(unsupported)} axis (this SRT has no gripper)."),
      )
    requested = set(axes)
  ...
  def _srt_read_home_sensor(self, axis: Axis) -> bool:
    ...
    try:
      ...
      logger.warning("SRT homing %s: 0x10 read failed; assuming off-sensor", axis.label)
      return False
    if len(resp) < 3:
      return False
