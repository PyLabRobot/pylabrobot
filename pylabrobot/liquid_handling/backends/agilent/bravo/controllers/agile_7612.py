...
from pylabrobot.liquid_handling.backends.agilent.bravo.controllers.base import AxisMoveInfo, JogParams
from pylabrobot.liquid_handling.backends.agilent.bravo.protocol.agile_packet import AGILE_PACKET_SIZE
...
from pylabrobot.liquid_handling.backends.agilent.bravo.protocol.agile_7612_commands import Agile7612MoveInfo
...
from pylabrobot.liquid_handling.backends.agilent.bravo.types import Axis, DeviceStateFlag, HeadType, SpeedLevel
...
class Agile7612Controller(AgileController):
    ...
    def _agile_7612_move_go(self, axes: list[Axis]) -> None:
        ...
        for axis in axes:
            ...
            raw[9] = crc8_maxim(raw, 9)
            comm.send_command(CommandID.DIRECT_AGILE_COMMAND,
                              bytes(raw) + struct.pack("<B", axis.value))
        ...
        for m in moves:
            if not self._homed[m.axis.value]:
                raise BravoError(
                    ...
                    custom_text=(
                        f"{m.axis.name} axis not initialized; "
    ...
    def _agile_7612_wait_for_settled(self, axes: list[Axis], timeout_ms: int = 30_000) -> None:
            ...
            try:
                ...
                if poll_count % 50 == 0:
                    ...
                    logger.warning(
                        ...
                        elapsed, [a.name for a in axes], ", ".join(stuck_info),
                    )
        ...
        logger.warning(
            ...
            [a.name for a in axes], ", ".join(stuck_info_final),
        )
        raise BravoError(ErrorType.MOVE_TIMEOUT,
                         custom_text=f"Timed out: {[a.label for a in axes]} ({timeout_ms}ms)")
    ...
    def _homing_vel_accel(self, axis: Axis) -> tuple[float, float]:
        ...
        return (self._vel_to_ticks_per_ms(axis, vel_mms),
    ...
    def _agile_7612_home_single_axis(self, axis: Axis) -> None:
        ...
        logger.info("Agile7612 homing %s: vel=%.4f accel=%.6f depart=%d", axis.label, vel, accel, depart_dir)
        ...
        info = self._move_info_cls(
            axis=axis, position=large_ticks * depart_dir, velocity=vel, acceleration=accel,
            absolute_move=False, check_for_homed=False, home_complete_register=home_reg,
        )
        comm.send_command(CommandID.PREPARE_MOVE, info.pack())
        ...
        info = self._move_info_cls(
            axis=axis, position=large_ticks * (-depart_dir if phase1_settled else depart_dir),
        ...
        info = self._move_info_cls(
            axis=axis, position=large_ticks * phase3_dir, velocity=vel / 10.0, acceleration=accel,
            absolute_move=False, check_for_homed=False, home_complete_register=home_reg,
        )
        comm.send_command(CommandID.PREPARE_MOVE, info.pack())
        self._agile_7612_move_go([axis])
        ...
        try:
            info = self._move_info_cls(
                axis=axis, position=0.0, velocity=vel, acceleration=accel,
                absolute_move=True, check_for_homed=True, home_complete_register=home_reg,
            )
            comm.send_command(CommandID.PREPARE_MOVE, info.pack())
            self._agile_7612_move_go([axis])
    ...
    def _home_x(self) -> None:
        ...
        try:
            ...
            if len(resp) >= 3:
                ...
                logger.info("Agile7612 homing X: reg 0x10 sensor byte=0x%02X → %s",
                            sensor_byte, "on sensor" if on_sensor else "off sensor")
        except BravoError:
        ...
        if on_sensor:
            ...
            self._agile_7612_servo_write(0xA4, _SERVO_A4_INITIAL, axis)
            info = self._move_info_cls(
                axis=axis, position=-large_ticks, velocity=vel, acceleration=accel,
        ...
        self._agile_7612_servo_write(0xA3, _SERVO_A3_SWAPPED, axis)
        info = self._move_info_cls(
            axis=axis, position=large_ticks, velocity=vel / 10.0, acceleration=accel,
            absolute_move=False, check_for_homed=False,
            home_complete_register=home_reg,
        )
        comm.send_command(CommandID.PREPARE_MOVE, info.pack())
    ...
    def _home_y(self) -> None:
        ...
        try:
            ...
            if len(resp) >= 3:
                sensor_byte = resp[2]
                on_sensor = bool(sensor_byte & self._home_sensor_bitmask(axis))
                logger.info("Agile7612 homing Y: reg 0x10 sensor byte=0x%02X → %s",
                            sensor_byte, "on sensor" if on_sensor else "off sensor")
        except BravoError:
    ...
    def _home_z(self) -> None:
        ...
        try:
            ...
            if len(resp) >= 3:
                sensor_byte = resp[2]
                on_sensor = bool(sensor_byte & self._home_sensor_bitmask(axis))
                logger.info("Agile7612 homing Z: reg 0x10 sensor byte=0x%02X → %s",
                            sensor_byte, "on sensor" if on_sensor else "off sensor")
        except BravoError:
        ...
        info = self._move_info_cls(
            axis=axis, position=0.0, velocity=vel, acceleration=accel,
            absolute_move=True, check_for_homed=True,
            home_complete_register=home_reg,
        )
        comm.send_command(CommandID.PREPARE_MOVE, info.pack())
        ...
        raw[9] = crc8_maxim(raw, 9)
        comm.send_command(CommandID.DIRECT_AGILE_COMMAND,
                          bytes(raw) + struct.pack("<B", ctrl_base))
        self._agile_7612_wait_for_settled([axis], timeout_ms=30_000)
    ...
    def _home_w(self) -> None:
        ...
        try:
            ...
            if len(resp) >= 3:
                sensor_byte = resp[2]
                on_sensor = bool(sensor_byte & self._home_sensor_bitmask(axis))
                logger.info("Agile7612 homing W: reg 0x10 sensor byte=0x%02X → %s",
                            sensor_byte, "on sensor" if on_sensor else "off sensor")
        except BravoError:
    ...
    def _home_zg(self) -> None:
        ...
        try:
            ...
            info = self._move_info_cls(
                axis=Axis.G, position=0.0, velocity=g_vel, acceleration=g_accel,
                absolute_move=True, check_for_homed=True,
        ...
        info = self._move_info_cls(
            axis=axis, position=park_ticks, velocity=vel, acceleration=accel,
            absolute_move=True, check_for_homed=True,
            home_complete_register=home_reg,
        )
        comm.send_command(CommandID.PREPARE_MOVE, info.pack())
    ...
    def _home_xy_parallel(self) -> None:
        ...
        for axis in xy_axes:
            ...
            home_reg = self._home_reg_for_axis(axis)
            info = self._move_info_cls(
                axis=axis, position=_HOMING_DISTANCE_MM * self._ticks_per_unit.get(axis, 314.96) * direction,
                velocity=vel, acceleration=accel,
                absolute_move=False, check_for_homed=False,
                home_complete_register=home_reg,
            )
        ...
        for axis in xy_axes:
            ...
            info = self._move_info_cls(
                axis=axis, position=-_HOMING_DISTANCE_MM * self._ticks_per_unit.get(axis, 314.96) * direction,
                velocity=vel / 10.0, acceleration=accel,
                absolute_move=False, check_for_homed=False,
                home_complete_register=home_reg,
            )
            comm.send_command(CommandID.PREPARE_MOVE, info.pack())
    ...
    def _agile_7612_move_axis_to_zero(self, axis: Axis) -> None:
        ...
        self.move([AxisMoveInfo(axis=axis, position=0.0,
                                velocity=vel_mms, acceleration=accel_mms2,
                                absolute=True)])
    ...
    def jog(self, params: JogParams) -> float:
        ...
        if current_pos >= params.max_position:
            logger.warning(
                ...
                axis.name, current_pos, params.max_position,
            )
            return current_pos
        ...
        if params.tolerance > 0 and final_pos > params.max_position + params.tolerance:
            logger.warning(
                ...
                axis.name, final_pos, params.max_position, params.tolerance,
            )
    ...
    def tip_force_jog(self, axis: Axis, peak_current: float,
        ...
        if current_pos >= max_position:
            logger.warning(
                ...
                axis.name, current_pos, max_position,
            )
            return current_pos
        ...
        if approach_target > current_pos:
            ...
            self.move(
                [AxisMoveInfo(axis=axis, position=approach_target,
                              **(dict(zip(("velocity", "acceleration"),
                                          self._speed_for_level(axis, SpeedLevel.SAFE)))))],
                wait=True,
            )
        ...
        logger.warning(
            ...
            axis.name, approach_target, max_position, target_ticks, peak_current,
        )
    ...
    def detect_gripper(self) -> "GripperDetectionState":
        from pylabrobot.liquid_handling.backends.agilent.bravo.types import GripperDetectionState
        comm = self._require_connected()
