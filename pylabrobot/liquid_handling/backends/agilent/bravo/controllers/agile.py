...
class AgileController(BravoController):
        ...
        try:
            ...
            if value != UNIQUE_VALUE_EXPECTED:
                logger.error(
                    ...
                    controller_id, value, UNIQUE_VALUE_EXPECTED,
                )
                return False
    ...
    def _wait_for_in_position(
        ...
        raise BravoError(
            ...
            custom_text=(
                f"Timed out waiting for axes "
                f"{[a.label for a in axes]} ({timeout_ms} ms)"
            ),
        )
    ...
    def move(
        ...
        for m in moves:
            ...
            logger.debug(
                "Prepare move: %s pos=%.1f ticks vel=%.4f ticks/ms "
                "accel=%.6f ticks/ms² abs=%s",
                m.axis.label, info.position, info.velocity,
                info.acceleration, info.absolute_move,
            )
            try:
