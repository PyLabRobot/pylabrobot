...
class ParameterAccess:
  ...
  def read_uint(self, param_id: int, timeout_ms: int = 5000) -> int:
    with self._lock:
      ...
      value = self._engine.get_value(self._address, CommonSubCommands.PARAM_DB_VALUE, timeout_ms)
  ...
  def read_float(self, param_id: int, timeout_ms: int = 5000) -> float:
    with self._lock:
      self._ensure_read_ptr(param_id, timeout_ms)
      value = self._engine.get_float(self._address, CommonSubCommands.PARAM_DB_VALUE, timeout_ms)
      self._last_read_ptr = param_id
      return value
  ...
  def write_uint(self, param_id: int, value: int, timeout_ms: int = 5000) -> None:
    with self._lock:
      ...
      self._engine.set_uint(self._address, CommonSubCommands.PARAM_DB_VALUE, value, timeout_ms)
      self._last_write_ptr = param_id

  def write_float(self, param_id: int, value: float, timeout_ms: int = 5000) -> None:
    with self._lock:
      self._ensure_write_ptr(param_id, timeout_ms)
      self._engine.set_float(self._address, CommonSubCommands.PARAM_DB_VALUE, value, timeout_ms)
      self._last_write_ptr = param_id
  ...
  def apply(self, timeout_ms: int = 10_000) -> None:
    ...
    self._engine.set_uint(self._address, CommonSubCommands.PARAM_DB_APPLY, 1, timeout_ms)

  def reset(self, timeout_ms: int = 10_000) -> None:
    """Reset parameters to defaults (SUBCMD_PARAM_DB_RESET)."""
    self._engine.set_uint(self._address, CommonSubCommands.PARAM_DB_RESET, 1, timeout_ms)

  def save(self, timeout_ms: int = 10_000) -> None:
    """Save parameters to flash (SUBCMD_PARAM_DB_SAVE)."""
    self._engine.set_uint(self._address, CommonSubCommands.PARAM_DB_SAVE, 1, timeout_ms)

  def load(self, timeout_ms: int = 10_000) -> None:
    """Load parameters from flash (SUBCMD_PARAM_DB_LOAD)."""
    self._engine.set_uint(self._address, CommonSubCommands.PARAM_DB_LOAD, 1, timeout_ms)

  def count(self, timeout_ms: int = 5000) -> int:
    """Return the controller's count of parameters in its database."""
    return self._engine.get_value(self._address, CommonSubCommands.PARAM_DB_COUNT, timeout_ms)
  ...
  def _ensure_read_ptr(self, param_id: int, timeout_ms: int) -> None:
    ...
    if self._last_read_ptr + 1 != param_id:
      self._engine.set_uint(self._address, CommonSubCommands.PARAM_DB_RD_PTR, param_id, timeout_ms)

  def _ensure_write_ptr(self, param_id: int, timeout_ms: int) -> None:
    if self._last_write_ptr + 1 != param_id:
      self._engine.set_uint(self._address, CommonSubCommands.PARAM_DB_WR_PTR, param_id, timeout_ms)
