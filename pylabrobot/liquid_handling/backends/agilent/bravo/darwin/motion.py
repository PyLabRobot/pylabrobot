...
def trigger_event(engine: GeminiEngine, event_number: int, timeout_ms: int = 5000) -> None:
  ...
  while remaining:
    ...
    if remaining and elapsed_ms > busy_confirm_ms:
      ...
      if missing:
        raise BravoError(
          ErrorType.MOVE_TIMEOUT,
          custom_text=(f"Axes never entered BUSY within {busy_confirm_ms}ms: {', '.join(missing)}"),
        )
