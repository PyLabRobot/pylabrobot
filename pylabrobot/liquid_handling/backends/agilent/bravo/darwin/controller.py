...
from pylabrobot.liquid_handling.backends.agilent.bravo.controllers.base import JogParams as BaseJogParams
...
from pylabrobot.liquid_handling.backends.agilent.bravo.darwin.topology import axis_address, axis_node, all_axes
from pylabrobot.liquid_handling.backends.agilent.bravo.darwin.waxis_config import config_for_head, ul_to_mm
from pylabrobot.liquid_handling.backends.agilent.bravo.darwin.waxis_params import apply_waxis_parameters
...
from pylabrobot.liquid_handling.backends.agilent.bravo.protocol.gemini.instruction import pack_float32
from pylabrobot.liquid_handling.backends.agilent.bravo.protocol.gemini.packet import MASTER_ADDRESS, InstructionAddress
...
class DarwinController(BravoController):
        ...
        if not self._engine.is_connected:
            ...
            if address and address != getattr(self._engine._transport, "_address", None):
                self._engine = GeminiEngine(
                    address, getattr(self._engine._transport, "_port", 7613)
                )
    ...
    def _post_connect_init(self) -> None:
        ...
        for a, state in self._axes.items():
            ...
            try:
                ...
                logger.warning(
                    "INSTR_CLEAR on %s failed (non-fatal): %s", a.name, exc
                )
            ...
            if a in _PEAK_CURRENT_AXES:
                try:
                    ...
                    logger.info(
                        ...
                        a.name, state.peak_current_max,
                    )
                except Exception as exc:
                    logger.warning(
                        "Could not read %s I2T_PEAK_CURRENT: %s", a.name, exc
                    )
        ...
        def _read_version(addr: InstructionAddress) -> str:
            try:
                packed = self._engine.get_value(
                    addr, CommonSubCommands.FW_VERSION, timeout_ms=5000
                )
            except Exception:
                return ""
    ...
    def _ensure_waxis_params(self) -> None:
        ...
        if (
            self._head_type == HeadType.HT_UNKNOWN
            or self._head_type == self._waxis_applied_head
        ):
            return
    ...
    def move(
        ...
        for m in moves:
            ...
            state = axis_module.read_motor_state(
                self._engine, axis_address(m.axis), timeout_ms=2000,
            )
            from pylabrobot.liquid_handling.backends.agilent.bravo.protocol.gemini.enums import MotorState as _MS
        ...
        for m in moves:
            ...
            velocity_pct = 100.0 if (m.velocity <= 0 or limits.velocity <= 0) \
                else min(100.0, m.velocity * 100.0 / limits.velocity)
            accel_pct = 100.0 if (m.acceleration <= 0 or limits.acceleration <= 0) \
            ...
            current_normalized = state.calibration.to_normalized(
                self.get_position(m.axis)
            )
            direction = (
            ...
            from pylabrobot.liquid_handling.backends.agilent.bravo.protocol.gemini.enums import InstructionTypes
            requests.append(motion.MoveRequest(
        ...
        motion.move_multi(
            self._engine, requests, wait=wait, timeout_ms=timeout_ms
        )
    ...
    def is_axis_homed(self, axis: Axis) -> bool:
        ...
        """
        try:
            return axis_module.is_initialized(
                self._engine, axis_address(axis)
            )
        except Exception:
            return False
    ...
    def clear_go_button(self) -> None:
        self._engine.master_set_uint(
            DarwinMasterNodeSubCommands.CLEAR_GO_BTN_LATCH, 1, timeout_ms=2000
        )
        ...
        for a in axes:
            ...
            if state == _MS.DISABLED:
                try:
                    ...
                    logger.warning(
                        "recover: enable %s failed: %s", a.name, exc
                    )
    ...
    def clear_lights(self) -> None:
        self._engine.master_set_uint(
            DarwinMasterNodeSubCommands.STATUS_LIGHTS, 0, timeout_ms=2000
        )
    ...
    def detect_smart_head(self) -> bool:
        ...
        try:
            self._engine.master_set_uint(
                DarwinMasterNodeSubCommands.SMART_INIT, 0, timeout_ms=2000
            )
            return True
    ...
    def grip(
        self, speed: SpeedLevel, position: float, grip_lid: bool = False
    ) -> None:
        ...
        sequences.grip(
            self._engine, g_addr, g_state.params,
    ...
    def is_plate_in_gripper(self) -> bool:
        ...
        if self._profile is not None:
            transient_ms = int(
                getattr(getattr(self._profile, "safety", None),
                        "plate_sensor_transient_ms", 0) or 0
            )
        try:
    ...
    def jog(self, params: BaseJogParams) -> float:
        ...
        final_normalized = sequences.jog(
            self._engine, addr, state.params,
            sequences.JogParams(
                axis_name=axis.name,
        ...
        for i in range(attempts):
            ...
            try:
                value = self._engine.get_value(
                    addr, GeminiSubCommands.PLATE_PRESENT, timeout_ms=5000,
                )
            except Exception as exc:
    ...
    def read_plate_sensor(self, transient_ms: int = 0) -> bool:
        ...
        try:
            ...
            result = self._read_plate_sensor_state(
                max_attempts=3, retry_delay_ms=100, retry_until_present=True,
            )
        finally:
        ...
        *,
        ...
        def _zg_move(target: float) -> None:
            ...
            timeout_ms = _motion_timeout_ms(
                distance, velocity_mm, min_ms=4000, margin_ms=1000,
            )
            self.move(
                [AxisMoveInfo(axis=Axis.Zg, position=target,
                              velocity=velocity_mm, acceleration=accel_mm,
                              absolute=True)],
                wait=True,
        ...
        start_timeout_ms = _motion_timeout_ms(
            start_distance, velocity_mm, min_ms=6000, margin_ms=2000,
        )
        self.move(
            [AxisMoveInfo(axis=Axis.Zg, position=start_zg,
                          velocity=velocity_mm, acceleration=accel_mm,
                          absolute=True)],
            wait=True,
        ...
        try:
            ...
            if not initial["read"]:
                detail = (
                    "; errors=" + " | ".join(initial["errors"])
                    if initial["errors"] else ""
                )
                raise BravoError(
                    ErrorType.COULD_NOT_QUERY_STATE,
                    custom_text=(
                        "Could not read plate sensor state from Darwin "
                        f"during scan{detail}"
                    ),
                )
            sensor_read_count += 1
            ...
            while present and self.get_position(Axis.Zg) > -20.0:
                ...
                back = self._read_plate_sensor_state(
                    max_attempts=3, retry_delay_ms=10,
                )
                if back["read"]:
                    sensor_read_count += 1
            ...
            while self.get_position(Axis.Zg) < end_zg:
                ...
                step_read = self._read_plate_sensor_state(
                    max_attempts=3, retry_delay_ms=10, retry_until_present=True,
                )
                if step_read["read"]:
                    sensor_read_count += 1
        ...
        if sensor_read_count <= 0:
            detail = (
                "; errors=" + " | ".join(sensor_read_errors)
                if sensor_read_errors else ""
            )
            raise BravoError(
                ErrorType.COULD_NOT_QUERY_STATE,
                custom_text=(
                    "Could not read plate sensor state from Darwin master "
                    f"during scan{detail}"
                ),
            )
    ...
    def get_state_snapshot(self, max_age_s: float = 0.15) -> dict[str, Any]:
        ...
        if (
            ...
            and (now - self._last_snapshot_at) <= max_age_s
        ):
    ...
    def send_command(
        self, command_id: int, data: bytes = b"", timeout_ms: int = 2000
    ) -> bytes:
        ...
        raise BravoError(
            ...
            custom_text=(
                f"DARWIN command passthrough is not implemented for "
                f"0x{int(command_id):02X}."
            ),
        )
    ...
    def aspirate(
        ...
        *,
        ...
        self.move([AxisMoveInfo(
            axis=Axis.W, position=target_mm,
            velocity=velocity_mm, acceleration=acceleration_mm, absolute=True,
...
def _encode_light_value(command: LightCommandData) -> int:
    ...
    match colors & 0x07:
        case 1: red = 100
        case 2: red = 25; green = 100
        case 3: red = 100; green = 65
        case 4: green = 100
        case 5: red = 100; green = 100
        case 6: red = 20; green = 100
        case 7: red = 80; green = 100
