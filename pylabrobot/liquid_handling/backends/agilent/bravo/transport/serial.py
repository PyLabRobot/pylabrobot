...
class SerialTransport(Transport):
  ...
  def connect(self) -> None:
    if self._serial and self._serial.is_open:
      return
    logger.info(
      "Connecting via serial: port=%s baud=%d hw_flow=%s", self._port, self._baud, self._hw_flow
    )
  ...
  def receive_exact(self, num_bytes: int, timeout_ms: int = 2000) -> bytes:
    ...
    while len(buf) < num_bytes:
      ...
      if remaining_time <= 0:
        raise TimeoutError(f"Timed out reading {num_bytes} bytes on {self._port} (got {len(buf)})")
