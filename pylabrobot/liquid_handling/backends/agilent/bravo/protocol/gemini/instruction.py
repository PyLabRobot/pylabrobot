...
from pylabrobot.liquid_handling.backends.agilent.bravo.protocol.gemini.enums import AxisDirection, InstructionTypes
...
class Instruction:
    ...
    def to_words(self) -> tuple[int, int, int, int]:
        ...
        word0 = (
            (int(self.instr_type) & 0xFF)
            | ((vel_scaled & 0xFFFF) << 8)
            | ((accel_scaled & 0xFF) << 24)
        )
        ...
        return (word0 & 0xFFFFFFFF, word1 & 0xFFFFFFFF,
                self.to_value & 0xFFFFFFFF, self.trig_at_value & 0xFFFFFFFF)
    ...
    def from_words(cls, w0: int, w1: int, w2: int, w3: int) -> "Instruction":
        ...
        inst = cls(
            instr_type=InstructionTypes(instr_type_value) if instr_type_value in InstructionTypes._value2member_map_ else instr_type_value,
