...
def pack_multipacket_batch(packets: list[Packet]) -> bytes:
    ...
    if len(packets) > MAX_PACKETS_PER_MULTIPACKET:
        raise ValueError(
            f"multipacket exceeds {MAX_PACKETS_PER_MULTIPACKET}-packet limit "
            f"(got {len(packets)})"
        )
    buf = bytearray()
    for p in packets:
    ...
    if len(buf) > MAX_MULTIPACKET_SIZE:
        raise ValueError(
            f"multipacket payload exceeds {MAX_MULTIPACKET_SIZE}-byte limit "
            f"(got {len(buf)})"
        )
    return bytes(buf)
...
def unpack_multipacket_batch(payload: bytes) -> list[Packet]:
    ...
    return [Packet.from_bytes(payload[i:i + PACKET_SIZE])
...
class MultipacketResponse:
    ...
    def from_bytes(cls, data: bytes) -> "MultipacketResponse":
        if len(data) < _MP_RESPONSE_SIZE:
            raise ValueError(
                f"MultipacketResponse requires {_MP_RESPONSE_SIZE} bytes, got {len(data)}"
            )
