...
from pylabrobot.liquid_handling.backends.agilent.bravo.protocol.gemini.instruction import (
  pack_float32,
  unpack_float32,
)
...
class GeminiEngine:
  ...
  def wait_for_trigger_event(self, event_value: int, timeout_ms: int) -> bool:
  ...
  def send_serial(self, payload: bytes, timeout_ms: int = 1000) -> bytes:
    ...
    with self._command_lock:
      ...
      while True:
        ...
        if remaining <= 0:
          raise GeminiTimeoutError("Gemini serial-packet timeout", timeout_ms=timeout_ms)
  ...
  def _rx_loop(self) -> None:
    ...
    try:
      while not self._rx_stop.is_set():
        ...
        try:
          header_bytes = self._transport.receive_exact(FRAME_HEADER_SIZE, timeout_ms=_RX_POLL_MS)
        except TimeoutError:
          continue
        ...
        if header.payload_size > 0:
          try:
            payload = self._transport.receive_exact(header.payload_size, timeout_ms=1000)
          except TimeoutError:
            logger.warning(
  ...
  def master_get_uint(self, sub_command: int, timeout_ms: int = 5000) -> int:
    from pylabrobot.liquid_handling.backends.agilent.bravo.protocol.gemini.packet import (
      MASTER_ADDRESS,
    )
  ...
  def master_set_uint(self, sub_command: int, value: int, timeout_ms: int = 5000) -> None:
    from pylabrobot.liquid_handling.backends.agilent.bravo.protocol.gemini.packet import (
      MASTER_ADDRESS,
    )
  ...
  def master_get_float(self, sub_command: int, timeout_ms: int = 5000) -> float:
    return unpack_float32(self.master_get_uint(sub_command, timeout_ms))

  def master_set_float(self, sub_command: int, value: float, timeout_ms: int = 5000) -> None:
