...
class OpentronsChatterboxTests(AnyioTestBase):
  ...
  async def _enter_lifespan(self, stack):
    ...
    stack.callback(lambda: set_tip_tracking(False))
    stack.callback(lambda: set_volume_tracking(False))
  ...
  async def test_setup_resolves_two_channels_without_ot_api(self):
    """chatterbox resolves both mounted pipettes."""
...
class OpentronsChatterboxVsSimulatorTests(AnyioTestBase):
  """Differential audit: the chatterbox must produce the same tracked outcome as
  the reference OpentronsOT2Simulator on the single-channel overlap."""
  ...
  async def test_chatterbox_matches_simulator_single_channel(self):
    ...
    try:
      ...
      self.assertEqual(simulator_outcome, chatterbox_outcome)
    finally:
