...
class TestSTARResponseParsing(unittest.TestCase):
    ...
    def test_parse_response_slave_errors(self):
        ...
        self.assertEqual(
            e.errors["Pipetting channel 16"].message, "Tip already picked up"
        )
...
class TestSTARLiquidHandlerCommands(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncSetUp(self):
        ...
        self.deck.assign_child_resource(
            self.bb, location=Coordinate(425, 141.5, 120 - 1)
        )
    ...
    def _assert_command_in_command_buffer(self, cmd: str, should_be: bool, fmt: str):
        ...
        if should_be and not found:
            if similar is not None:
                ...
                self.fail(
                    f"Command {cmd} not found in sent commands: {self.mockSTAR.commands}"
                )
        elif not should_be and found:
            self.fail(
                f"Command {cmd} was found in sent commands: {self.mockSTAR.commands}"
            )
    ...
    async def test_aspirate56(self):
        ...
        await self.lh.aspirate(
            self.plate["A1", "B1"],
            vols=[corrected_vol, corrected_vol],
            use_channels=[4, 5],
    ...
    async def test_single_channel_aspiration(self):
        ...
        await self.lh.aspirate(
            [well],
            vols=[self.hlc.compute_corrected_volume(100)],
    ...
    async def test_multi_channel_aspiration(self):
        ...
        corrected_vol = self.hlc.compute_corrected_volume(100)
        await self.lh.aspirate(
            self.plate["A1:B1"], vols=[corrected_vol] * 2, **self.hlc.make_asp_kwargs(2)
        )
    ...
    async def test_core_96_aspirate(self):
        ...
        await self.lh.aspirate96(
            self.plate, volume=corrected_volume, **hlc.make_asp96_kwargs()
        )
    ...
    async def test_core_96_dispense(self):
        ...
        corrected_volume = hlc.compute_corrected_volume(100)
        await self.lh.aspirate96(
            self.plate, corrected_volume, **hlc.make_asp96_kwargs()
        )
    ...
    async def test_move_core(self):
        ...
        self._assert_command_sent_once(
            "C0ZPid0021xs03479xd0yj1142yv0050zj1876zy0500yo0885yg0825yw15"
            "th2450te2450",
    ...
    async def test_iswap_pick_up_resource_grip_direction_changes_plate_width(self):
        ...
        size_y = 200
        plate = Plate(
            "dummy", size_x=size_x, size_y=size_y, size_z=100, ordered_items={}
        )
        ...
        with unittest.mock.patch.object(
            ...
            assert (
                mocked_iswap_get_plate.call_args.kwargs["plate_width"]
                == size_x * 10 - 33
            )
