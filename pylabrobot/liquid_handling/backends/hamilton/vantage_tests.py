...
class TestVantageResponseParsing(unittest.TestCase):
    ...
    def test_parse_error_response(self):
        ...
        self.assertEqual(
            error,
            VantageFirmwareError(
                errors={"Cover": "Slave not available"}, raw_response=resp
            ),
        )
        ...
        self.assertEqual(
            error,
            VantageFirmwareError(
                errors={"Cover": "S-Drive: Drive not initialized"}, raw_response=resp
            ),
        )
...
class TestVantageLiquidHandlerCommands(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_aspirate(self):
        ...
        await self.lh.aspirate(
            self.plate["A1"], vols=[corrected_volume], **hlc.make_asp_kwargs(1)
        )
