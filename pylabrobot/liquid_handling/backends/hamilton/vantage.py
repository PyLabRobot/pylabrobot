...
class Vantage(HamiltonLiquidHandler):
    ...
    async def setup(
        ...
        if not pip_channels_initialized or any(tip_presences):
            await self.pip_initialize(
                ...
                begin_z_deposit_position=[int(self._traversal_height * 10)]
                * self.num_channels,
        ...
        loading_cover_initialized = (
            await self.loading_cover_request_initialization_status()
        )
    ...
    async def pick_up_tips(
        ...
        x_positions, y_positions, tip_pattern = self._ops_to_fw_positions(
            ops, use_channels
        )
        ...
        try:
            return await self.pip_tip_pick_up(
                ...
                * len(ops),
                end_z_deposit_position=[round((max_z + max_tip_length) * 10)]
                * len(ops),
        ...
        try:
            return await self.pip_tip_discard(
                ...
                * len(ops),
                tip_handling_method=[
                    0 for _ in ops
                ],  # Always appears to be 0, even in trash.
    ...
    async def aspirate(
        ...
        return await self.pip_aspirate(
            ...
            * len(ops),
            ...
            immersion_depth=[
                round(id_ * 10) for id_ in immersion_depth or [0] * len(ops)
            ],
            ...
            transport_air_volume=[
                round(tav * 10) for tav in transport_air_volume or [0] * len(ops)
            ],
            ...
            mix_position_in_z_direction_from_liquid_surface=[
                round(mp)
                for mp in mix_position_in_z_direction_from_liquid_surface
                or [0] * len(ops)
            ],
    ...
    async def dispense(
        ...
        return await self.pip_dispense(
            ...
            immersion_depth=[
                round(id * 10) for id in immersion_depth or [0] * len(ops)
            ],
            surface_following_distance=[
                round(sfd * 10)
                for sfd in surface_following_distance or [2.1] * len(ops)
            ],
            tube_2nd_section_height_measured_from_zm=[
                round(t2sh * 10)
                for t2sh in tube_2nd_section_height_measured_from_zm or [0] * len(ops)
            ...
            minimal_height_at_command_end=[
                round(mh * 10)
                for mh in minimal_height_at_command_end
                or [self._traversal_height] * len(ops)
            ],
            ...
            stop_back_volume=[
                round(sbv * 100) for sbv in stop_back_volume or [0] * len(ops)
            ],
            transport_air_volume=[
                round(tav * 10) for tav in transport_air_volume or [0] * len(ops)
            ],
    ...
    async def pick_up_tips96(
        ...
        return await self.core96_tip_pick_up(
            ...
            minimal_traverse_height_at_begin_of_command=round(
                (minimal_traverse_height_at_begin_of_command or self._traversal_height)
                * 10
            ),
    ...
    async def aspirate96(
        ...
        if isinstance(aspiration, AspirationPlate):
            ...
            lld_search_height = (
                well_bottoms + top_left_well.get_absolute_size_z() + 2.7 - 1
            )
        else:
            position = (
            ...
            bottom = position.z
            lld_search_height = (
                bottom + aspiration.container.get_absolute_size_z() + 2.7 - 1
            )
    ...
    async def dispense96(
        ...
        if isinstance(dispense, DispensePlate):
            ...
            bottom = position.z
            lld_search_height = (
                bottom + dispense.container.get_absolute_size_z() + 2.7 - 1
            )
    ...
    async def move_channel_y(self, channel: int, y: float):
        ...
        return await self.position_single_channel_in_y_direction(
            channel + 1, round(y * 10)
        )

    async def move_channel_z(self, channel: int, z: float):
        """Move the specified channel to the specified z coordinate."""

        return await self.position_single_channel_in_z_direction(
            channel + 1, round(z * 10)
        )
    ...
    async def define_tip_needle(
        ...
        if not 1 <= tip_length <= 1999:
            raise ValueError(
                "tip_length must be between 1 and 1999, but is " f"{tip_length}"
            )
    ...
    async def pip_aspirate(
        ...
        if minimal_traverse_height_at_begin_of_command is None:
            ...
            0 <= x <= 3600 for x in minimal_traverse_height_at_begin_of_command
        ):
            raise ValueError(
        ...
        if aspirate_position_above_z_touch_off is None:
            ...
            raise ValueError(
                "aspirate_position_above_z_touch_off must be in range 0 to 100"
            )
        ...
        if capacitive_mad_supervision_on_off is None:
            ...
            raise ValueError(
                "capacitive_mad_supervision_on_off must be in range 0 to 1"
            )
    ...
    async def pip_dispense(
        ...
        if dispense_position_above_z_touch_off is None:
            ...
            raise ValueError(
                "dispense_position_above_z_touch_off must be in range 0 to 100"
            )
    ...
    async def grip_plate(
        ...
        if not -50000 <= plate_center_x_direction <= 50000:
            raise ValueError(
                "plate_center_x_direction must be in range -50000 to 50000"
            )
