...
class HamiltonLiquidHandler(LiquidHandlerBackend, USBBackend, metaclass=ABCMeta):
        ...
        for channel_involved in tip_pattern:
            if channel_involved:
                if arg_index >= len(val):
                    raise ValueError(
                        f"Too few values for tip pattern {tip_pattern}: {val}"
                    )
        ...
        for k, v in kwargs.items():
            if type(v) is datetime.datetime:
                ...
                v = " ".join([str(e) for e in v]) + (
                    "&" if len(v) < self.num_channels else ""
                )
            if k.endswith("_"):  # workaround for kwargs named in, as, ...
                k = k[:-1]
            assert len(k) == 2, (
                "Keyword arguments should be 2 characters long, but got: " + k
            )
            cmd += f"{k}{v}"
        ...
        while len(self._waiting_tasks) > 0:
            for id_, (loop, fut, cmd, timeout_time) in self._waiting_tasks.items():
                if time.time() > timeout_time:
                    logger.warning(
                        "Timeout while waiting for response to command %s.", cmd
                    )
                    loop.call_soon_threadsafe(
                        fut.set_exception,
                        TimeoutError(
                            f"Timeout while waiting for response to command {cmd}."
                        ),
                    )
    ...
    def _ops_to_fw_positions(
        ...
        for i, channel in enumerate(use_channels):
            ...
            x_pos = (
                ops[i].resource.get_absolute_location(x="c", y="c", z="b").x
                + ops[i].offset.x
            )
            x_positions.append(round(x_pos * 10))

            y_pos = (
                ops[i].resource.get_absolute_location(x="c", y="c", z="b").y
                + ops[i].offset.y
            )
        ...
        for channel_idx1, (x1, y1) in enumerate(zip(x_positions, y_positions)):
            for channel_idx2, (x2, y2) in enumerate(zip(x_positions, y_positions)):
                if channel_idx1 == channel_idx2:
                    continue
                if (
                    not channels_involved[channel_idx1]
                    or not channels_involved[channel_idx2]
                ):
                    continue
                if (
                    x1 != x2
                    ...
                    continue
                if (
                    not (self.allow_firmware_planning and y1 == y2)
                    and abs(y1 - y2) < 90
                ):
                    raise ValueError(
        ...
        if len(ops) > self.num_channels:
            raise ValueError(
                f"Too many channels specified: {len(ops)} > {self.num_channels}"
            )
    ...
    async def get_or_assign_tip_type_index(self, tip: HamiltonTip) -> int:
        ...
        if tip_type_hash not in self._tth2tti:
            ...
            await self.define_tip_needle(
                ...
                tip_length=round(
                    (tip.total_tip_length - tip.fitting_depth) * 10
                ),  # in 0.1mm
