...
class SerializingBackendTests(unittest.TestCase):
  ...
  def setUp(self) -> None:
    self.lh = LiquidHandler(backend=SerializingSavingBackend(), deck=STARLetDeck())
    self.lh.setup()
  ...
  def test_pick_up_tips(self):
    ...
    self.assertEqual(self.lh.backend.sent_commands[0]["command"], "pick_up_tips")
    self.assertEqual(self.lh.backend.sent_commands[0]["data"], dict(
      channels=[Pickup(resource=tips[0]).serialize()]))

  def test_discard_tips(self):
    tips = self.tips["A1"]
    self.lh.discard_tips(tips)
    self.assertEqual(len(self.lh.backend.sent_commands), 1)
    self.assertEqual(self.lh.backend.sent_commands[0]["command"], "discard_tips")
    self.assertEqual(self.lh.backend.sent_commands[0]["data"], dict(
      channels=[Discard(resource=tips[0]).serialize()]))

  def test_aspirate(self):
    wells = self.plate["A1"]
    self.lh.aspirate(wells, vols=10, liquid_classes=None)
    self.assertEqual(len(self.lh.backend.sent_commands), 1)
    self.assertEqual(self.lh.backend.sent_commands[0]["command"], "aspirate")
    self.assertEqual(self.lh.backend.sent_commands[0]["data"], dict(
      channels=[Aspiration(resource=wells[0], volume=10).serialize()]))

  def test_dispense(self):
    wells = self.plate["A1"]
    self.lh.dispense(wells, vols=10, liquid_classes=None)
    self.assertEqual(len(self.lh.backend.sent_commands), 1)
    self.assertEqual(self.lh.backend.sent_commands[0]["command"], "dispense")
    self.assertEqual(self.lh.backend.sent_commands[0]["data"], dict(
      channels=[Dispense(resource=wells[0], volume=10).serialize()]))

  def test_pick_up_tips96(self):
    self.lh.pick_up_tips96(self.tips)
    self.assertEqual(len(self.lh.backend.sent_commands), 1)
    self.assertEqual(self.lh.backend.sent_commands[0]["command"], "pick_up_tips96")
    self.assertEqual(self.lh.backend.sent_commands[0]["data"], dict(resource=self.tips.serialize()))

  def test_discard_tips96(self):
    self.lh.discard_tips96(self.tips)
    self.assertEqual(len(self.lh.backend.sent_commands), 1)
    self.assertEqual(self.lh.backend.sent_commands[0]["command"], "discard_tips96")
    self.assertEqual(self.lh.backend.sent_commands[0]["data"], dict(resource=self.tips.serialize()))

  def test_aspirate96(self):
    self.lh.aspirate_plate(self.plate, volume=10, liquid_class=None)
    self.assertEqual(len(self.lh.backend.sent_commands), 1)
    self.assertEqual(self.lh.backend.sent_commands[0]["command"], "aspirate96")
    self.assertEqual(self.lh.backend.sent_commands[0]["data"], dict(aspiration=
  ...
  def test_dispense96(self):
    ...
    self.assertEqual(len(self.lh.backend.sent_commands), 1)
    self.assertEqual(self.lh.backend.sent_commands[0]["command"], "dispense96")
    self.assertEqual(self.lh.backend.sent_commands[0]["data"], dict(dispense=
  ...
  def test_move(self):
    ...
    print([cmd["command"] for cmd in self.lh.backend.sent_commands])
    self.assertEqual(len(self.lh.backend.sent_commands), 3)
    self.assertEqual(self.lh.backend.sent_commands[0]["command"], "move")
    self.assertEqual(self.lh.backend.get_first_data_for_command("move"), dict(move=
      Move(
