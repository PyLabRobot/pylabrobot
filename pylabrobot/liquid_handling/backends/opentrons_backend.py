...
class OpentronsBackend(LiquidHandlerBackend):
    ...
    def num_channels(self) -> int:
        return len(
            [p for p in [self.left_pipette, self.right_pipette] if p is not None]
        )
    ...
    async def assigned_resource_callback(self, resource: Resource):
        ...
        well_definitions = {
            child.name: {
                ...
                + child.get_absolute_size_x() / 2,
                "y": cast(Coordinate, child.location).y
                + child.get_absolute_size_y() / 2,
    ...
    def select_tip_pipette(
        self, tip_max_volume: float, with_tip: bool
    ) -> Optional[str]:
        ...
        if self.left_pipette is not None:
            left_volume = OpentronsBackend.pipette_name2volume[
                self.left_pipette["name"]
            ]
        ...
        if self.right_pipette is not None:
            right_volume = OpentronsBackend.pipette_name2volume[
                self.right_pipette["name"]
            ]
            if (
                right_volume == tip_max_volume
    ...
    async def pick_up_tips(self, ops: List[Pickup], use_channels: List[int]):
        ...
        assert use_channels == [
            0
        ], "manual channel selection not supported on OT for now"
        ...
        labware_id = self.defined_labware[
            op.resource.parent.name
        ]  # get name of tip rack
        tip_max_volume = op.tip.maximal_volume
        pipette_id = self.select_tip_pipette(tip_max_volume, with_tip=False)
        if not pipette_id:
            raise NoChannelError(
                "No pipette channel of right type with no tip available."
            )
        ...
        if (
            ...
            and pipette_id == self.left_pipette["pipetteId"]
        ):
    ...
    def get_pipette_name(self, pipette_id: str) -> str:
        ...
        if (
            ...
            and pipette_id == self.right_pipette["pipetteId"]
        ):
    ...
    async def move_resource(self, move: Move):
        ...
        raise NotImplementedError(
            "Moving resources in Opentrons is not implemented yet."
        )
