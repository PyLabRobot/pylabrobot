...
class WebSocketBackend(SerializingBackend):
    ...
    def __init__(
        ...
        if not HAS_WEBSOCKETS:
            raise RuntimeError(
                "The WebSocketBackend requires websockets to be installed."
            )
    ...
    async def send_command(
        ...
        if self.has_connection():
            asyncio.run_coroutine_threadsafe(
                self.websocket.send(serialized_data), self.loop
            )
            ...
            while True:
                try:
                    async with websockets.server.serve(
                        ...
                        print(
                            f"Websocket server started at http://{self.ws_host}:{self.ws_port}"
                        )
                        lock.release()
                        await self.stop_
                        break
