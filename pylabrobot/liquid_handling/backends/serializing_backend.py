...
class SerializingBackend(LiquidHandlerBackend, metaclass=ABCMeta):
    ...
    async def unassigned_resource_callback(self, name: str):
        await self.send_command(
            command="resource_unassigned", data={"resource_name": name}
        )
    ...
    async def move_channel_x(self, channel: int, x: float):
        await self.send_command(
            command="move_channel_x", data={"channel": channel, "x": x}
        )

    async def move_channel_y(self, channel: int, y: float):
        await self.send_command(
            command="move_channel_y", data={"channel": channel, "y": y}
        )

    async def move_channel_z(self, channel: int, z: float):
        await self.send_command(
            command="move_channel_z", data={"channel": channel, "z": z}
        )
