...
def check_contaminated(liquid_history_tip, liquid_history_well):
    ...
    return (
        not liquid_history_tip.issubset(liquid_history_well)
        and len(liquid_history_tip) > 0
    )
            ...
            if does_volume_tracking():
                if not op.resource.tracker.is_disabled:
                    ...
                    if check_updatable(op.tip.tracker, op.resource.tracker):
                        op.tip.tracker.liquid_history.update(
                            op.resource.tracker.liquid_history
                        )
    ...
    async def aspirate96(
        ...
        **backend_kwargs,
        ...
        if isinstance(resource, Container):
            if (
                resource.get_absolute_size_x() < 108.0
                or resource.get_absolute_size_y() < 70.0
            ...
            aspiration = AspirationContainer(
                ...
                liquids=cast(
                    List[List[Tuple[Optional[Liquid], float]]], all_liquids
                ),  # stupid
            )
        else:
