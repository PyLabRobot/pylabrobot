...
class LiquidHandler(Resource, Machine):
    ...
    def __init__(
        ...
        # added to this value.
        self.default_offset_head96: Coordinate = (
            default_offset_head96 or Coordinate.zero()
        )
    ...
    async def setup(self, **backend_kwargs):
        ...
        self.head = {
            c: TipTracker(thing=f"Channel {c}")
    ...
    def _check_args(
        ...
        if len(missing) > 0:
            raise TypeError(
                f"Missing arguments to backend.{method.__name__}: {missing}"
            )
        ...
        if len(extra) > 0 and len(vars_keyword) == 0:
            if strictness == Strictness.STRICT:
                raise TypeError(
                    f"Extra arguments to backend.{method.__name__}: {extra}"
                )
            elif strictness == Strictness.WARN:
                warnings.warn(f"Extra arguments to backend.{method.__name__}: {extra}")
            else:
                logger.debug(
                    "Extra arguments to backend.%s: %s", method.__name__, extra
                )
    ...
    def _normalize_backend_kwargs(
        self, backend_kwargs, n_ops: int, index: int | None = None
    ):
        ...
        if sequential and len(use_channels) > 1:
            ...
            for i, (ch, op) in enumerate(zip(use_channels, ops)):
                normalized_kwargs = self._normalize_backend_kwargs(
                    backend_kwargs, n_ops=1, index=i
                )

                await fn(ops=[op], use_channels=[ch], **normalized_kwargs)
        else:
            normalized_kwargs = self._normalize_backend_kwargs(
                backend_kwargs, n_ops=len(ops)
            )
    ...
    async def pick_up_tips(
        ...
        **backend_kwargs,
        ...
        # fix arguments
        use_channels = (
            use_channels or self._default_use_channels or list(range(len(tip_spots)))
        )
        ...
        if error is not None and isinstance(error, ChannelizedError):
            successes = [
                channel_idx not in error.errors for channel_idx in use_channels
            ]
        ...
        for channel, op, success in zip(use_channels, pickups, successes):
            if does_tip_tracking() and not op.resource.tracker.is_disabled:
                (
                    ...
                    if success
                    else op.resource.tracker.rollback
                )()
    ...
    def get_mounted_tips(self) -> List[Optional[Tip]]:
        ...
        """
        return [
            tracker.get_tip() if tracker.has_tip else None
    ...
    async def drop_tips(
        ...
        **backend_kwargs,
        ...
        if len(not_tip_spots) > 0:
            raise TypeError(
                f"Resources must be `TipSpot`s or Trash, got {not_tip_spots}"
            )
    ...
    async def discard_tips(
        ...
        **backend_kwargs,
        ...
        if n == 0:
            raise RuntimeError(
                "No tips have been picked up and no channels were specified."
            )
        ...
        **backend_kwargs,
        ...
        if len(set(resources)) == 1:
            ...
            if spread == "tight":
                ...
                raise ValueError(
                    "Invalid value for 'spread'. Must be 'tight', 'wide', or 'custom'."
                )
        ...
        for channel, op, success in zip(use_channels, aspirations, successes):
            if does_volume_tracking():
                ...
                (
                    tip_volume_tracker.commit
                    if success
                    else tip_volume_tracker.rollback
                )()
                ...
                for requested_bav, done_bav in zip(
                    ...
                    if (
                        ...
                        raise BlowOutVolumeError(
                            "Blowout volume is larger than aspirated volume"
                        )
        ...
        **backend_kwargs,
        ...
        if target_vols is not None:
            if ratios is not None:
                raise TypeError(
                    "Cannot specify ratios and target_vols at the same time"
                )
            if source_vol is not None:
                raise TypeError(
                    "Cannot specify source_vol and target_vols at the same time"
                )
        else:
    ...
    async def pick_up_tips96(
        ...
        **backend_kwargs,
        ...
        for i, tip_spot in enumerate(tip_rack.get_all_items()):
            ...
            if tip_spot.has_tip():
                self.head96[i].add_tip(
                    tip_spot.get_tip(), origin=tip_spot, commit=False
                )
            ...
            if (
                does_tip_tracking()
                and not tip_spot.tracker.is_disabled
                and tip_spot.has_tip()
            ):
    ...
    async def drop_tips96(
        ...
        **backend_kwargs,
        ...
        for i in range(96):
            ...
            if (
                tip.tracker.get_used_volume() > 0
                and not allow_nonzero_volume
                and does_volume_tracking()
            ):
        ...
        **backend_kwargs,
        ...
        if not (
            ...
            or (
                ...
                and all(isinstance(w, Well) for w in resource)
            )
        ):
            raise TypeError(
        ...
        tips = [
            channel.get_tip() if channel.has_tip else None
        ...
        if isinstance(resource, Plate):
            ...
            containers = (
                resource.get_all_items()
                if resource.num_items > 1
                else [resource.get_item(0)]
            )
        ...
        if len(containers) == 1:  # single container
            ...
            if not len(containers) == 96:
                raise ValueError(
                    f"aspirate96 expects 96 containers when a list, got {len(containers)}"
                )
        ...
        **backend_kwargs,
        ...
        for tip in tips:
            ...
            if does_volume_tracking():
                ...
                tip.tracker.remove_liquid(
                    volume=min(tip.tracker.get_used_volume(), volume)
                )
        ...
        if len(containers) == 1:  # single container
            ...
            if not container.tracker.is_disabled and does_volume_tracking():
                container.tracker.add_liquid(
                    volume=len([t for t in tips if t is not None]) * volume
                )
    ...
    async def stamp(
        ...
        await self.dispense96(
            resource=source, volume=volume, flow_rate=dispense_flow_rate
        )
    ...
    async def pick_up_resource(
        ...
        **backend_kwargs,
        ...
        if self._resource_pickup is not None:
            raise RuntimeError(
                f"Resource {self._resource_pickup.resource.name} already picked up"
            )
    ...
    async def drop_resource(
        ...
        **backend_kwargs,
        ...
        destination_rotation = (
            destination.get_absolute_rotation().z
            if not isinstance(destination, Coordinate)
            else 0
        )
        resource_rotation_wrt_destination = (
        ...
        if isinstance(destination, ResourceStack):
            ...
            to_location = destination.get_location_wrt(
                self.deck
            ...
            if (
                destination.resource is not None
                and destination.resource is not resource
            ):
            ...
            to_location = (
                destination.get_location_wrt(self.deck) + adjusted_plate_anchor
            )
        ...
        if isinstance(destination, Coordinate):
            ...
            destination, (ResourceStack, PlateReader)
            ...
            destination.assign_child_resource(
                resource, location=destination.child_location
            )
        elif isinstance(destination, PlateAdapter):
            if not isinstance(resource, Plate):
                raise ValueError("Only plates can be moved to a PlateAdapter")
    ...
    def deserialize(cls, data: dict, allow_marshal: bool = False) -> LiquidHandler:
        ...
        if "default_offset_head96" in data:
            default_offset = deserialize(
                data["default_offset_head96"], allow_marshal=allow_marshal
            )
    ...
    async def probe_tip_presence_via_pickup(
        ...
        for idx, tip_spot in enumerate(tip_spots):
            assert (
                tip_spot.location is not None
            ), "TipSpot location must be at a location"
        ...
        for cluster in sorted_clusters:
            ...
            if any(presence_flags[index] for index in index_subset):
                ...
                try:
                    ...
                    assert (
                        cluster[0][0].location is not None
                    ), "TipSpot location must be at a location"
                    print(
                        f"Warning: drop_tips failed for cluster at x={cluster[0][0].location.x}: {e}"
                    )
    ...
    async def consolidate_tip_inventory(
        ...
        def merge_sublists(
            lists: List[List[TipSpot]], max_len: int
        ) -> List[List[TipSpot]]:
        ...
        for idx, tip_rack in enumerate(tip_racks):
            ...
            tip_status = [
                tip_spot.tracker.has_tip for tip_spot in tip_rack.get_all_items()
            ]
            ...
            tipspots_w_tips = [
                tip_spot
                for has_tip, tip_spot in zip(tip_status, tip_rack.get_all_items())
                if has_tip
            ]
            ...
            clusters_by_model.setdefault(current_model, []).append(
                (tip_rack, num_empty_tipspots)
            )
        ...
        for model, rack_list in clusters_by_model.items():
            ...
            all_tip_spots_list = [
                tip_spot
                for tip_rack, _ in rack_list
                for tip_spot in tip_rack.get_all_items()
            ]

            # 1: Record current tip state
            current_tip_presence_list = [
                tip_spot.has_tip() for tip_spot in all_tip_spots_list
            ]
            ...
            tip_movement_list = [
                c - t
                for c, t in zip(current_tip_presence_list, target_tip_presence_list)
            ]

            tip_origin_indices = [i for i, v in enumerate(tip_movement_list) if v == 1]
            all_origin_tip_spots = [
                all_tip_spots_list[idx] for idx in tip_origin_indices
            ]

            tip_target_indices = [i for i, v in enumerate(tip_movement_list) if v == -1]
            all_target_tip_spots = [
                all_tip_spots_list[idx] for idx in tip_target_indices
            ]
            ...
            if num_channels_available == 0:
                raise ValueError(
                    f"No channel capable of handling tips on deck: {current_tip_model}"
                )

            # by aggregating drop columns i.e. same drop column should not be visited twice!
            if (
                num_channels_available >= 8
            ):  # physical constraint of tip_rack's having 8 rows
                ...
                merged_target_tip_clusters = list(
                    divide_list_into_chunks(
                        all_target_tip_spots, chunk_size=num_channels_available
                    )
                )
            ...
            for idx, target_tip_spots in enumerate(merged_target_tip_clusters):
                ...
                origin_tip_spots = [
                    all_origin_tip_spots.pop(0) for _ in range(len(target_tip_spots))
                ]
