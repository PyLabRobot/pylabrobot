...
class LiquidHandler(Machine):
  ...
  async def dispense(
    ...
    **backend_kwargs,
    ...
    if does_volume_tracking():
      liquids = [
        c.get_tip().tracker.get_liquids(top_volume=vol) for c, vol in zip(self.head.values(), vols)
      ]
    else:
  ...
  async def transfer(
    ...
    **backend_kwargs,
    ...
    await self.aspirate(
      resources=[source], vols=[sum(target_vols)], flow_rates=aspiration_flow_rate, **backend_kwargs
    )
  ...
  async def discard_tips96(self, allow_nonzero_volume: bool = True, **backend_kwargs):
    ...
    return await self.drop_tips96(
      self.deck.get_trash_area96(), allow_nonzero_volume=allow_nonzero_volume, **backend_kwargs
    )
