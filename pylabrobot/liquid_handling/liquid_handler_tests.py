...
class TestLiquidHandlerLayout(unittest.IsolatedAsyncioTestCase):
    ...
    def test_get_resource(self):
        ...
        self.assertEqual(
            self.lh.deck.get_resource("plate carrier").name, "plate carrier"
        )

        # Get subresource.
        self.assertEqual(self.lh.deck.get_resource("tip_rack_01").name, "tip_rack_01")
        self.assertEqual(
            self.lh.deck.get_resource("aspiration plate").name, "aspiration plate"
        )
    ...
    async def test_move_plate_to_site(self):
        ...
        self.assertEqual(
            plate.get_item("A1").get_absolute_location()
            + plate.get_item("A1").center(),
    ...
    async def test_move_plate_rotation(self):
        ...
        sites: List[Union[ResourceStack, PlateCarrierSite]] = [
            ...
            PlateCarrierSite(
                name="site", size_x=100, size_y=100, size_z=15, pedestal_size_z=1
            ),
        ]
    ...
    def test_serialize(self):
        ...
        self.assertEqual(
            deserialized.backend.__class__.__name__, self.lh.backend.__class__.__name__
        )
...
class TestLiquidHandlerCommands(unittest.IsolatedAsyncioTestCase):
        ...
        self.assertEqual(
            self.get_first_command("aspirate"),
            {
                ...
                "kwargs": {
                    "use_channels": [0],
                    "ops": [
                        _make_asp(well, vol=10, offset=Coordinate(x=1, y=1, z=1), tip=t)
                    ],
                },
            },
        )
        self.assertEqual(
            self.get_first_command("dispense"),
            {
                ...
                "kwargs": {
                    ...
                    "ops": [
                        _make_disp(
                            well, vol=10, offset=Coordinate(x=1, y=1, z=1), tip=t
                        )
                    ],
                },
            },
    ...
    async def test_return_tips96(self):
        ...
        self.assertEqual(
            self.get_first_command("drop_tips96"),
            {
                ...
                "kwargs": {
                    "drop": DropTipRack(
                        resource=self.tip_rack, offset=Coordinate.zero()
                    )
                },
            },
        )
    ...
    async def test_transfer(self):
        ...
        await self.lh.transfer(
            self.plate.get_well("A1"), self.plate["A2"], source_vol=10
        )
        ...
        dispenses = list(
            filter(lambda x: x["command"] == "dispense", self.backend.commands_received)
        )
        self.assertEqual(
            dispenses,
            [
        ...
        self.assertEqual(
            self.get_first_command("drop_tips"),
            {
                ...
                "kwargs": {
                    ...
                    "ops": [
                        Drop(
                            self.deck.get_trash_area(), tip=tips[3], offset=offsets[0]
                        ),
                        Drop(
                            self.deck.get_trash_area(), tip=tips[2], offset=offsets[1]
                        ),
                        Drop(
                            self.deck.get_trash_area(), tip=tips[1], offset=offsets[2]
                        ),
                        Drop(
                            self.deck.get_trash_area(), tip=tips[0], offset=offsets[3]
                        ),
                    ],
                },
            },
