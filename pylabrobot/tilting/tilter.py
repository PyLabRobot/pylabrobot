...
class Tilter(ResourceHolderMixin, Machine):
    ...
    def experimental_get_plate_drain_offsets(
        ...
        angle = (
            absolute_angle
            if self._hinge_coordinate.x < self._size_x / 2
            else -absolute_angle
        )
    ...
    def experimental_get_well_drain_offsets(
        self, wells: List[Well], n_tips: int = 1, absolute_angle: Optional[float] = None
    ) -> List[Coordinate]:
        ...
        angle = absolute_angle * (
            -1 if self._hinge_coordinate.x >= self._size_x / 2 else 1
        )
        ...
        return [
            offset for well_offsets in well_drain_offsets for offset in well_offsets
        ]
