...
class SubsystemError(STARModuleError):
  """Subsystem error

  Possible cause(s):
    This error code indicates an error in one of the subsystems. (for error handling purpose using service
    software macro code)
...
class LiquidLevelError(STARModuleError):
  ...
  This error is created from main / subsystem error 06/70, 06/73 and 06/87.
...
class NotAspiratedError(STARModuleError):
  ...
  This error is created from main / subsystem error 02/54.
...
class ImproperDispensationError(STARModuleError):
  ...
  This error is created from main / subsystem error 02/52 and 02/54.
...
def star_firmware_string_to_error(
  ...
  for module_id, error in error_code_dict.items():
    ...
    if "/" in error:
      ...
      # Subsystem modules: er## (just trace information)
  ...
  # If the master error is a SubsystemError, remove it from the errors dict.
  if isinstance(errors.get("Master"), SubsystemError):
    errors.pop("Master")
