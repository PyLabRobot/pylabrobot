...
class _FirmwareLock:
  ...
  Two layers, both handled by ``subsystem(module)`` / ``c0()`` so callers never touch the
  internals:

  * A readers/writer gate between the C0 master and the subsystem modules. A subsystem
    command blocks the C0 master while in flight; a C0 master command (``c0()``) is
    *exclusive*: it waits for all subsystem commands to drain, then runs alone. So no
    subsystem command ever overlaps a C0 master command.
  ...
  * A per-module mutex so at most one command per module is in flight. Different modules
  ...
  The first subsystem command acquires the exclusive lock and the last one releases it,
  so a C0 command simply takes the same lock and automatically waits the subsystems out.
  """

  # Subsystem modules that serialize against themselves: H0 (96-head), X0 (X-drives),
  ...
  def __init__(self):
    ...
    # Readers/writer gate: subsystem commands vs the exclusive C0 master.
    self._subsystem_count = 0
    self._subsystem_count_lock = asyncio.Lock()
  ...
  @asynccontextmanager
  async def subsystem(self, module: str):
    """Run a subsystem command: serialize on its module mutex and block the C0 master."""
    module_lock = self._module_lock(module)
    async with AsyncExitStack() as stack:
      if module_lock is not None:
        await stack.enter_async_context(module_lock)
      # Join the subsystem group; first in acquires the exclusive lock, last out releases it.
      async with self._subsystem_count_lock:
        self._subsystem_count += 1
        if self._subsystem_count == 1:
      ...
      try:
        ...
        async with self._subsystem_count_lock:
          self._subsystem_count -= 1
          if self._subsystem_count == 0:
            self._exclusive_lock.release()

  @asynccontextmanager
  async def c0(self):
    """Run an exclusive C0 master command. Waits for all subsystem commands, then runs alone."""
