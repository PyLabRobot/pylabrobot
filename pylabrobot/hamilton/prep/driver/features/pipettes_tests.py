...
def test_aspirate_sends_an_explicit_zero_blow_out_and_refuses_a_zero_flow_rate():
  """A given 0.0 blow-out is sent over the class's; a flow rate of 0.0 is refused before sending."""

  from pylabrobot.hamilton.star.liquid_classes.mapping import (
    StandardVolumeFilter_Water_DispenseJet_Empty as water,
  )
...
def test_aspirate_scales_the_profile_to_a_surface_following_distance():
  ...
  async def _t():
    ...
    await p.pipettes.dispense(
      [well], vols=[20.0], use_channels=[0], liquid_height=[3.0], disable_volume_correction=[True]
    )
    sent.clear()
    await p.pipettes.aspirate(
      [well],
...
def _empty_setup(second_session: bool):
  """A simulated Prep with a 50 uL tip rack; every command recorded with the link it went on."""
  deck = PrepDeck()
...
def test_empty_tip_moves_the_piston_to_rest_on_its_own_drive_and_empties_the_tracker():
  ...
  async def _t():
    p, rack, sent = _empty_setup(False)
    await p.setup()
    assert p.pipettes is not None
    await p.pipettes.pick_up_tips(rack["A1:B1"], use_channels=[0, 1])
...
def test_empty_tips_empties_every_channel_holding_a_tip(second_session):
  ...
  async def _t():
    p, rack, sent = _empty_setup(second_session)
    await p.setup()
    assert p.pipettes is not None
    await p.pipettes.pick_up_tips(rack["A1:B1"], use_channels=[0, 1])
    sent.clear()
...
def test_consolidating_moves_the_last_tips_into_the_first_empty_spots_two_at_a_time():
  ...
  async def _t():
    ...
    try:
      ...
      picked = [c for _, c in sent if isinstance(c, PrepCmd.PrepPickUpTips)]
      dropped = [c for _, c in sent if isinstance(c, PrepCmd.PrepDropTips)]
      assert [len(c.tip_positions) for c in picked] == [2] * 5
      assert [len(c.tip_positions) for c in dropped] == [2] * 5
    finally:
      set_tip_tracking(False)
    await p.stop()
  ...
  async def _t():
    ...
    try:
      ...
      with pytest.raises(ValueError, match="mixed tip models"):
        await p.pipettes.consolidate_tip_inventory([rack])
      assert [c for _, c in sent if isinstance(c, PrepCmd.PrepPickUpTips)] == []
    finally:
      set_tip_tracking(False)
    await p.stop()
