...
def parse_vantage_fw_string(s: str, fmt: Optional[Dict[str, str]] = None) -> dict:
  ...
  Example:
    >>> parse_vantage_fw_string("id0xs30 -100 +1 1000", {"xs": "[int]"})
    {"id": 0, "xs": [30, -100, 1, 1000]}

    >>> parse_vantage_fw_string('id0es"error string"', {"es": "str"})
    {"id": 0, "es": "error string"}
  """
  ...
  fmt = dict(fmt)  # copy to avoid mutating the caller's dict
