...
class STAR(Device):
  ...
  async def setup(self):
    ...
    if self._driver.iswap is not None:
      self.iswap = OrientableArm(backend=self._driver.iswap, reference_resource=self.deck)
  ...
  async def core_grippers(
    ...
    await self._driver.pick_up_core_gripper_tools(
      x_position=xs,
    ...
    backend = CoreGripper(driver=self._driver)
    arm = GripperArm(backend=backend, reference_resource=self.deck, grip_axis="y")

    try:
      yield arm
    finally:
      await self._driver.return_core_gripper_tools(
        x_position=xs,
        back_channel_y=back_y,
        front_channel_y=front_y,
