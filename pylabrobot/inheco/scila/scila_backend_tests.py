...
class TestSCILADriver(unittest.IsolatedAsyncioTestCase):
  def setUp(self):
  ...
  async def test_request_drawer_status(self):
    ...
    positions = await self.driver.request_drawer_statuses()
    self.assertEqual(
      positions,
      {
        1: "Opened",
  ...
  async def test_request_valve_status(self):
    ...
    status = await self.driver.request_valve_status()
    self.assertEqual(
      status,
      {
        "H2O": "Opened",
...
class TestSCILATemperatureBackend(unittest.IsolatedAsyncioTestCase):
  def setUp(self):
    ...
    self.MockInhecoSiLAInterface.return_value = self.mock_sila_interface
    self.driver = SCILADriver(scila_ip="127.0.0.1")
    self.backend = SCILATemperatureBackend(driver=self.driver)
