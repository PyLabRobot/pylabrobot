...
from pylabrobot.molecular_devices.imageXpress.pico.models import (
  Exposure,
  FocalPosition,
  Gain,
  ImagingMode,
...
class Pico:
  ...
  async def setup(self) -> None:
    ...
    for pos, obj in self._objectives.items():
      ...
      await self.change_objective(pos, obj.objective_id)
    ...
    for pos, mode in self._filter_cubes.items():
      ...
      await self.change_filter_cube(pos, mode.filter_cube)
  ...
  async def capture(
    ...
    light_channel = mode.light_channel
    filter_cube = mode.filter_cube
    excitation_source = mode.excitation_source
    objective_str = objective.objective_id
