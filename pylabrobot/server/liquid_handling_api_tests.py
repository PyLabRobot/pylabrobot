...
class LiquidHandlingApiGeneralTests(unittest.IsolatedAsyncioTestCase):
    ...
    def test_load_labware(self):
        with self.app.test_client() as client:
            ...
            self.assertEqual(response.status_code, 400)
            self.assertEqual(
                response.json, {"error": "missing key in json data: 'deck'"}
            )
...
class LiquidHandlingApiOpsTests(unittest.TestCase):
    ...
    def test_aspirate(self):
        ...
        self.test_tip_pickup()  # pick up a tip first
        with self.app.test_client() as client:
            well = cast(Plate, self.lh.deck.get_resource("aspiration plate")).get_item(
                "A1"
            )
            task = client.post(
    ...
    def test_config(self):
        cfg = Config(
            logging=Config.Logging(log_dir=Path("logs"), level=logging.CRITICAL)
        )
        with self.app.test_client() as client:
