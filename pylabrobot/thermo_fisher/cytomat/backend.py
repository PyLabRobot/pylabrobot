...
from pylabrobot.thermo_fisher.cytomat.errors import (
  CytomatBusyError,
...
from pylabrobot.thermo_fisher.cytomat.schemas import (
  ActionRegisterState,
  OverviewRegisterState,
  SensorStates,
  SwapStationState,
)
from pylabrobot.thermo_fisher.cytomat.utils import (
  hex_to_base_twelve,
  hex_to_binary,
...
class CytomatBackend(
  ...
  def _site_to_firmware_string(self, site: PlateHolder) -> str:
    ...
    rack_idx = [rack.name for rack in self._racks].index(rack.name)
  ...
  async def initialize(self) -> None:
    await self.send_action("ll", "in", "", timeout=300)
