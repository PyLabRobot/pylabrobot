...
# ---------------------------------------------------------------------------
# ThermoFisherThermocyclerDriver — all SCPI I/O lives here
# ---------------------------------------------------------------------------
...
class ThermoFisherThermocyclerDriver:
  ...
  async def get_block_current_temperature(self, block_id: int) -> List[float]:
  ...
  async def get_sample_temps(self, block_id: int) -> List[float]:
  ...
  async def block_ramp_single_temp(self, target_temp: float, block_id: int, rate: float = 100):
    ...
    It might be better to use ``set_block_temperature`` to set individual temperatures for each
    zone.
    """
    if block_id not in self.available_blocks:
      raise ValueError(f"Block {block_id} not available")
  ...
  async def open_lid(self, block_id: int):
    if self.bid != "31":
  ...
  async def close_lid(self, block_id: int):
    if self.bid != "31":
      raise NotImplementedError("Lid control is only available for BID 31 (ATC)")
...
class ThermoFisherBlockBackend(TemperatureControllerBackend):
  ...
  async def set_temperature(self, temperature: float):
    ...
    await self._driver.set_block_temperature(temperature=temps, block_id=self._block_id)

  async def get_current_temperature(self) -> float:
    temps = await self._driver.get_block_current_temperature(block_id=self._block_id)
    return float(temps[0])

  async def deactivate(self):
    await self._driver.deactivate_block(block_id=self._block_id)
...
class ThermoFisherLidBackend(TemperatureControllerBackend):
  ...
  async def set_temperature(self, temperature: float):
    temps = [temperature] * self._driver.num_temp_zones
    await self._driver.set_lid_temperature(temperature=temps, block_id=self._block_id)

  async def get_current_temperature(self) -> float:
    temps = await self._driver.get_lid_current_temperature(block_id=self._block_id)
    return float(temps[0])

  async def deactivate(self):
    await self._driver.deactivate_lid(block_id=self._block_id)
...
class ThermoFisherThermocyclingBackend(ThermocyclingBackend):
  ...
  async def open_lid(self) -> None:
    ...
    await self._driver.open_lid(block_id=self._block_id)

  async def close_lid(self) -> None:
    if not self._supports_lid_control:
      raise NotImplementedError("Lid control is not supported on this thermocycler model")
    await self._driver.close_lid(block_id=self._block_id)
  ...
  async def get_current_cycle_index(self) -> int:
    return await self._driver.get_current_cycle_index(block_id=self._block_id)

  async def get_current_step_index(self) -> int:
    return await self._driver.get_current_step_index(block_id=self._block_id)
