...
from ot_api.modules import (
  ...
  thermocycler_run_profile,
)
from ot_api.runs import enqueue_command
...
class OpentronsThermocyclerBackend(ThermocyclerBackend):
  ...
  **Note**: Gen-2 via HTTP-API does not expose a separate lid target field,
  ...
  async def get_block_target_temperature(self) -> float:
    return cast(float, self._find_module()["targetTemperature"])
  ...
  async def get_lid_target_temperature(self) -> float:
    """Always None on Gen-2 HTTP-API."""
    return cast(float, self._find_module().get("lidTargetTemperature"))
