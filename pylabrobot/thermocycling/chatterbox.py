...
class ThermocyclerChatterboxBackend(ThermocyclerBackend):
    ...
    for stage_idx, stage in enumerate(protocol.stages):
      ...
      for repeat_idx in range(stage.repeats):
        ...
        for step_idx, step in enumerate(stage.steps):
          ...
          # In a real implementation, you would wait here or handle timing
  ...
  async def get_hold_time(self) -> float:
    ...
    for i in range(self._state.total_steps):
      completed_step = self._state.protocol[i]
      ...
      if i < self._state.total_steps - 1:
        next_step = self._state.protocol[i + 1]
    ...
    final_temp = self._state.protocol[-1].temperature
