...
class PumpArray(Machine):
    ...
    async def run_continuously(
        ...
        if isinstance(use_channels, list) and len(set(use_channels)) != len(
            use_channels
        ):
            ...
            tasks = [
                asyncio.create_task(
                    self.run_for_duration(
                        speed=channel_speed, use_channels=channel, duration=duration
                    )
                )
                for channel_speed, channel, duration in zip(
                    speed, use_channels, durations
                )
            ]
            ...
            tasks = [
                asyncio.create_task(
                    self.run_revolutions(
                        num_revolutions=num_rotation, use_channels=channel
                    )
                )
