...
class PumpCalibration:
    ...
    def __init__(
        ...
        if any(value <= 0 for value in calibration):
            raise ValueError(
                "A value in the calibration is is outside expected parameters."
            )
    ...
    def load_calibration(
        ...
        if isinstance(calibration, (float, int)):
            if num_items is None:
                raise ValueError(
                    "num_items must be specified if calibration is a value."
                )
    ...
    def deserialize(cls, data: dict) -> PumpCalibration:
        return cls(
            calibration=data["calibration"], calibration_mode=data["calibration_mode"]
        )
    ...
    def load_from_json(
        ...
        raise TypeError(
            f"Calibration pulled from {file_path} is not a dictionary or list."
        )
    ...
    def load_from_dict(
        ...
        if sorted(calibration.keys()) != list(range(len(calibration))):
            raise ValueError(
                "Keys must be a contiguous range of integers starting at 0."
            )
