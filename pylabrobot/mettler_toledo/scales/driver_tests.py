...
class MTSICSResponseParsingTests(unittest.TestCase):
  ...
  def setUp(self):
    self.driver = MTSICSDriver.__new__(MTSICSDriver)
  ...
  def test_parse_errors_ES_ET_EL(self):
    ...
    with self.assertRaises(MettlerToledoError) as ctx:
      self.driver._parse_basic_errors(R("ES", ""))
    self.assertIn("Syntax error", str(ctx.exception))

    with self.assertRaises(MettlerToledoError) as ctx:
      self.driver._parse_basic_errors(R("ET", ""))
    self.assertIn("Transmission error", str(ctx.exception))

    with self.assertRaises(MettlerToledoError) as ctx:
      self.driver._parse_basic_errors(R("EL", ""))
  ...
  def test_parse_errors_status_codes(self):
    ...
    with self.assertRaises(MettlerToledoError) as ctx:
      self.driver._parse_basic_errors(R("S", "I"))
    self.assertIn("not executable at present", str(ctx.exception))

    with self.assertRaises(MettlerToledoError) as ctx:
      self.driver._parse_basic_errors(R("S", "L"))
    self.assertIn("incorrect parameter", str(ctx.exception))

    with self.assertRaises(MettlerToledoError) as ctx:
      self.driver._parse_basic_errors(R("S", "+"))
  ...
  def test_validate_response_rejects_short(self):
    ...
    with self.assertRaises(MettlerToledoError):
      MTSICSDriver._validate_response(R("I4", "A"), 3, "I4")

    # should not raise
    MTSICSDriver._validate_response(R("I4", "A", ["B207696838"]), 3, "I4")

  def test_validate_unit_rejects_wrong(self):
    """Non-gram unit responses must be rejected.
    The driver assumes grams throughout - a wrong unit would produce wrong values."""
    with self.assertRaises(MettlerToledoError):
      MTSICSDriver._validate_unit("kg", "S")

    # should not raise
    MTSICSDriver._validate_unit("g", "S")
  ...
  def test_parse_errors_passes_valid_success(self):
    ...
    self.driver._parse_basic_errors(R("Z", "A"))
  ...
  def test_parse_errors_weight_response_error(self):
    ...
    with self.assertRaises(MettlerToledoError) as ctx:
      self.driver._parse_basic_errors(R("S", "S", ["Error", "10b"]))
  ...
  def test_dataclass_construction(self):
    ...
    This is the foundation for all response access throughout the driver."""
