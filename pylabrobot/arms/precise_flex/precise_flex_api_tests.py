...
class PreciseFlexApiHardwareTests(unittest.IsolatedAsyncioTestCase):
  ...
  SAFE_LOCATION_C = (175, 0.003, 169.995, -0.001, 90, 180, 1)
  SAFE_LOCATION_J = (170.003, 0, 180, -180, 75.486)
  TEST_LOCATION_C_RIGHT = (271.692, 115.89, 169.969, 2.473, 90, 180, 1)
  TEST_LOCATION_J_RIGHT = (169.975, -16.736, 115.991, -110.057, 75.527)
  TEST_LOCATION_C_LEFT = (246.024, -147.689, 169.955, 6.879, 90, 180, 2)
  TEST_LOCATION_J_LEFT = (169.888, 10.079, 231.358, -240.229, 75.533)

  lock = asyncio.Lock()

  async def asyncSetUp(self):
    """Ensure connected, baseline the robot, capture station state."""
    ...
    current_position = await self.robot.where_j()
    if any(abs(a - b) > 1 for a, b in zip(current_position, self.SAFE_LOCATION_J)):
  ...
  async def asyncTearDown(self):
    ...
    try:
      ...
      # Restore pallet config if applicable
      if hasattr(self, "_original_station_location") and self._original_station_location[2] == 1:
        if hasattr(self, "_original_pallet_origin"):
          await self.robot.set_pallet_origin(*self._original_pallet_origin)
        if hasattr(self, "_original_pallet_final_x_pos"):
          await self.robot.set_pallet_x(*self._original_pallet_final_x_pos)
        if hasattr(self, "_original_pallet_final_y_pos"):
          await self.robot.set_pallet_y(*self._original_pallet_final_y_pos)
        if hasattr(self, "_original_pallet_final_z_pos"):
          await self.robot.set_pallet_z(*self._original_pallet_final_z_pos)
        if hasattr(self, "_original_pallet_index"):
          await self.robot.set_pallet_index(*self._original_pallet_index)

      # Restore station location
      if hasattr(self, "_original_station_location"):
...
#region LOCATION COMMANDS
  ...
  async def test_get_location_z_clearance(self) -> None:
    ...
    self.assertIsInstance(z_world, bool)
...
#region MOTION COMMANDS
  ...
  async def test_halt(self) -> None:
    """Test halt() command"""
    # Start a movement
    await self.robot.move_j(self.TEST_PROFILE_ID, *self.TEST_LOCATION_J_RIGHT)
  ...
  # async def test_where_c_loop(self) -> None:
  #   """Test where_c() command loop"""
  ...
  #   for i in range(20):
  #     await self.robot.move_c(self.TEST_PROFILE_ID, *self.TEST_LOCATION_C_RIGHT)
  #     await self.robot.move_c(self.TEST_PROFILE_ID, *self.TEST_LOCATION_C_LEFT)
  #     await self.robot.wait_for_eom()
  #     try:
  #       new_position = await self.robot.where_c()
  #     except Exception as e:
  #       print(f"Failed to get Cartesian position after move_c: {e}")
  #       await self.robot.wait_for_eom()
  #       new_position = await self.robot.where_c()
  #     print(f"POS_{i+1}: {new_position}")
...
#region SSGRIP COMMANDS
  ...
  async def test_release_plate(self) -> None:
    """Test release_plate()"""
    # Test basic release
    await self.robot.release_plate(100.0, 50)
    print("Release plate (basic) executed successfully")

    # Test release with InRange
    await self.robot.release_plate(120.0, 75, 5.0)
    print("Release plate (with InRange) executed successfully")

    # Test invalid finger speed
    with self.assertRaises(ValueError):
      await self.robot.release_plate(100.0, 0)  # Speed too low

    with self.assertRaises(ValueError):
      await self.robot.release_plate(100.0, 101)  # Speed too high
  ...
  async def test_pick_plate(self) -> None:
    ...
    # set test location
    await self.robot.set_location_xyz(self.TEST_LOCATION_ID, *self.TEST_LOCATION_C_LEFT[:-1])
    await self.robot.set_location_config(self.TEST_LOCATION_ID, self.TEST_LOCATION_C_LEFT[-1])
    await self.robot.set_location_z_clearance(self.TEST_LOCATION_ID, 20.0)
