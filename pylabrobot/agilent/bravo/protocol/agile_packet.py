"""Agile motor controller 10-byte packet codec (CRC-8/SMBUS variant).

This wire format is not vendor protocol documentation. It was recovered by
observing traffic between Agilent VWorks and a Bravo Agile controller
(Beckman AgileDrive), not from a published specification. The protocol has no
authentication or encryption: anyone with network access to the instrument's
TCP port can send it commands.

The Agile controller speaks a fixed 10-byte binary packet with a CRC-8
checksum. These packets are carried inside a V11 frame, one per
``CMD_DIRECT_AGILE_COMMAND`` (0xA1) command. Packet layout::

    byte 0     header / command type
    byte 1     controller ID (0 for broadcast)
    bytes 2-8  payload, meaning depends on the command
    byte 9     CRC8 checksum

The functions here build packets matching the vendor Agile SDK's own naming:
``register_get``, ``register_set_value``, ``move_absolute_value``,
``move_relative_value``, ``move_jog_value``, ``move_go``, ``servo_enable``,
``servo_disable``, ``reset_faults``, ``get_group_a_status``.

The Agile 7612 generation uses the same packet shape with a different
checksum; see :mod:`.agile_7612_packet`.
"""
...
# CRC8/SMBUS lookup table (polynomial 0x07).
_CRC8_TABLE = [
  0x00,
  0x07,
  0x0E,
...
def crc8(data: Union[bytes, bytearray], length: Optional[int] = None) -> int:
  """Compute the CRC-8/SMBUS checksum over a byte sequence.

  Args:
    data: The bytes to checksum.
    length: How many leading bytes of ``data`` to include. Defaults to all of
      ``data``; packet building passes 9, to checksum a 10-byte packet's
      first nine bytes while excluding its own CRC byte.

  Returns:
    The single checksum byte.
  """
  if length is None:
    length = len(data)
  crc = 0
...
def _make_packet(header: int, controller_id: int, payload: bytes) -> bytes:
  """Build a 10-byte Agile packet with a CRC-8/SMBUS checksum.
...
def verify_packet(packet: bytes) -> bool:
  """Verify a received 10-byte Agile packet's CRC-8/SMBUS checksum.
...
class AgileCommand(IntEnum):
  """Header byte values selecting an Agile packet's command type."""
...
class AgileRegister(IntEnum):
  """Register addresses read and written on the Agile controller."""
...
@dataclass
class AgileReply:
  """A parsed Agile response packet.
  ...
  def from_packet(cls, packet: bytes) -> AgileReply:
    """Parse a 10-byte Agile response packet.
