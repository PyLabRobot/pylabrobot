"""Wire-level constants and enums for the Gemini protocol.

These values are the vocabulary of the Darwin-generation controller-tree
protocol: which subcommand ID reads which parameter, which bit pattern in a
frame header means what, and how a controller-tree node address is composed.
They come from the firmware's own command table and are not something this
module derives; every value here must match what a Darwin controller board
actually implements on the wire.
"""
...
class TCPMessageType(IntEnum):
  """Outer TCP frame payload type (frame header bytes 4-5)."""
...
class CommandTypes(IntEnum):
  """``Packet.cmd_type`` -- the low 4 bits of packet byte 2."""
...
class SubCommandDataType(IntEnum):
  """How a subcommand's ``cmd_val`` word is interpreted."""
...
class CommonSubCommands(IntEnum):
  """Subcommands valid on every controller-tree node (master and axis). 0-18."""
...
class DarwinMasterNodeSubCommands(IntEnum):
  """Master-node subcommands specific to the Darwin controller. 19-33."""
...
class InstructionTypes(IntEnum):
  """Motion/logical instruction type -- encoded into instruction word0's low byte."""
...
class ExtraEventTriggerType(IntEnum):
  """How an extra-event trigger fires relative to a move."""
...
class FirmwareUpdateType(IntEnum):
  """Payload shape of a firmware-update file transfer."""
...
class FileCmd(IntEnum):
  """File-transfer operation selector for ``CommonSubCommands.FILE_CMD``."""
...
class MotorState(IntEnum):
  """BLDC axis lifecycle states, as written to and read from ``MOTOR_STATE``."""
...
class AxisDirection(IntEnum):
  """Direction bit of a Gemini instruction (word1 bit 16)."""
...
class ParamDBs(IntEnum):
  ...
  Addressed with ``CommonSubCommands.PARAM_DB_RD_PTR`` /
  ``PARAM_DB_WR_PTR`` to read or write one tuning/calibration parameter
  stored on a controller-tree node.
  """
...
# Well-known controller-tree addresses (InstructionAddress node_id)
NODE_BROADCAST = 63
NODE_MASTER = 1
...
class ReservedEvent(IntEnum):
  """Subcodes of ``EVENT_RESERVED`` (127), broadcast by the master node.

  Decoded from the high bits of a composite InstructionEvent value; see
  :func:`decode_instruction_event`.
  """
...
# How long a broadcast SET blocks before returning, in milliseconds, so the
# controller-tree bus has time to carry the broadcast to every node before the
# host issues its next command.
BROADCAST_WAIT_MS = 6


def decode_instruction_event(evt: int) -> tuple[bool, int, int]:
  """Decode an InstructionEvent word into its composite fields.

  Args:
    evt: The raw event value from a packet's ``cmd_val``.

  Returns:
    A ``(is_composite, event_no, mask)`` tuple: whether bit 7 (the composite
    flag) is set, the low-7-bit event number, and the mask carried in the
    high bits.
  """
...
def is_reserved_event(evt: int) -> Optional[ReservedEvent]:
  """Return the reserved-event subcode if ``evt`` is a composite RESERVED event.

  Args:
    evt: The raw event value from a packet's ``cmd_val``.

  Returns:
    The decoded :class:`ReservedEvent`, or ``None`` if ``evt`` is not a
    composite event with event number :data:`EVENT_RESERVED`, or its mask
    does not correspond to a known reserved subcode.
  """
