"""Structured errors for Bravo hardware and driver failures.

The Rabbit motion controller reports faults as a single status byte in its
response frame; this module turns that byte (or a locally detected failure,
such as a connection timeout) into a typed :class:`BravoError` that callers
can inspect programmatically instead of parsing message text.
"""
...
from .types import _AXIS_BY_CODE, Axis, axis_display_name, axis_label


class ErrorType(IntEnum):
  """All Bravo hardware and driver error categories."""
...
_ERROR_MESSAGES: dict[ErrorType, str] = {
  ...
  ErrorType.NODEZERO_NO_SERIAL_COMM: (
    "Node Zero does not support serial communication (use ethernet)."
  ),
...
class BravoError(Exception):
  """A structured error raised from the Bravo hardware or driver.

  Carries the :class:`ErrorType` category and, when the fault is
  axis-specific, the affected :data:`~pylabrobot.agilent.bravo.types.Axis`.
  """
  ...
  def __init__(
    ...
    """Create a structured Bravo error.

    Args:
      error_type: The error category.
      axis: The axis the error applies to, if any.
      custom_text: A message to use instead of the default for
        ``error_type``.
    """
  ...
  def __str__(self) -> str:
    ...
    if self.axis is not None:
      msg = f"{msg} ({axis_label(self.axis)})"
    return msg

  def __repr__(self) -> str:
    """Return an unambiguous, debugging-oriented representation."""
    parts = [f"error_type={self.error_type.name}"]
    if self.axis is not None:
      parts.append(f"axis={axis_display_name(self.axis)}")
    if self.custom_text:
...
def rabbit_error_to_bravo_error(code: int) -> BravoError:
  """Convert a Rabbit firmware error code to a :class:`BravoError`.

  Args:
    code: The raw error byte from a Rabbit response frame.

  Returns:
    The corresponding structured error. An unrecognized code maps to
    :attr:`ErrorType.UNKNOWN_RABBIT_ERROR`.
  """
  if code == RabbitErrorCode.NONE:
    return BravoError(ErrorType.NO_ERROR)
  if 0x20 <= code <= 0x25:
    axis = _AXIS_BY_CODE[code - 0x20]
