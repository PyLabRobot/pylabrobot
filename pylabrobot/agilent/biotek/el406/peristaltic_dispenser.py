...
class PeristalticDispenser:
  ...
  async def dispense(
    ...
    Dispenses into the plate currently loaded in the EL406 (see ``EL406.set_plate``).

    Args:
      volumes: Volume in uL per column index.
      flow_rate: Flow rate ("Low", "Medium", or "High").
      offset_x: X offset in mm (-12.5 to 12.5).
      offset_y: Y offset in mm (-4.0 to 4.0).
      offset_z: Z offset in mm (0.1-150.0). None picks the plate-type default:
    ...
    async with self._el406.batch():
      for vol, cols in groups:
        await self._peristaltic_dispense(
          plate,
          volume=vol,
          columns=cols,
          flow_rate=flow_rate,
  ...
  async def prime(
    ...
    ``prime()`` on :class:`PlateWasher` for buffer-specific priming.
    ...
    logger.info(
      "Peristaltic prime: %.1f uL, flow rate %s, cassette %s", wire_volume, flow_rate, cassette
    )
    ...
    async with self._el406.batch():
      await self._el406._send_step_command(
        framed_command, timeout=self._el406.timeout + wire_duration + 30
      )
  ...
  async def purge(
    ...
    validate_peristaltic_flow_rate(flow_rate)
    logger.info(
      "Peristaltic purge: %.1f uL, flow rate %s, cassette %s", wire_volume, flow_rate, cassette
    )
  ...
  def _validate_dispense_params(
    ...
    offset_x_steps = round(offset_x * 10)
    offset_y_steps = round(offset_y * 10)
    offset_z_steps = round(offset_z * 10) if offset_z is not None else None
    ...
    column_mask = self._validate_well_selection(plate, columns, rows)
    flow_rate_enum = PERISTALTIC_FLOW_RATE_MAP[flow_rate]
  ...
  async def _peristaltic_dispense(
    ...
    """
    offset_x_steps, offset_y_steps, offset_z_steps, flow_rate_enum, column_mask = (
      self._validate_dispense_params(
        plate,
        volume,
        columns,
        flow_rate=flow_rate,
    ...
    logger.info(
      "Peristaltic dispense: %.1f uL, flow rate %s, cassette %s", volume, flow_rate, cassette
    )
