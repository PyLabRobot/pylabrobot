...
class PlateWasher:
  """Manifold plate washer for the BioTek EL406."""

  def __init__(self, el406: EL406) -> None:
    self._el406 = el406
  ...
  def _validate_aspirate_params(
    ...
    resolved_z = offset_z if offset_z is not None else pt_defaults["aspirate_z"]
    ...
    if not vacuum_filtration:
      ...
      time_value, rate_byte = delay_ms, travel_rate_to_byte(travel_rate)
    else:
    ...
    # validate offsets
    self._validate_manifold_xy(offset_x, offset_y, "Aspirate")
    if not 1 <= resolved_z <= 210:
      raise ValueError(f"Aspirate Z offset must be 1-210, got {resolved_z}")
    if secondary_aspirate:
      self._validate_manifold_xy(secondary_x, secondary_y, "Secondary")
      if not 1 <= resolved_secondary_z <= 210:
        raise ValueError(f"Secondary Z offset must be 1-210, got {resolved_secondary_z}")

    return (resolved_z, resolved_secondary_z, time_value, rate_byte)
  ...
  def _validate_wash_params(
    ...
    *,
    ...
    (
      ...
      resolved_dispense_z,
      resolved_aspirate_z,
      resolved_secondary_z,
      resolved_final_secondary_z,
    ...
    self._validate_manifold_xy(dispense_x, dispense_y, "Wash dispense")
    validate_travel_rate(aspirate_travel_rate)
    self._validate_manifold_xy(aspirate_x, aspirate_y, "Wash aspirate")
    ...
    self._validate_manifold_xy(final_aspirate_x, final_aspirate_y, "Final aspirate")
    ...
    if final_secondary_aspirate:
      self._validate_manifold_xy(final_secondary_x, final_secondary_y, "Final secondary")
  ...
  async def aspirate(
    ...
    """
    resolved_z, resolved_secondary_z, time_value, rate_byte = self._validate_aspirate_params(
      self._el406.plate,
    ...
    data = self._build_aspirate_command(
      ...
      secondary_x=secondary_x,
      secondary_y=secondary_y,
      secondary_z=resolved_secondary_z,
    )
    framed_command = build_framed_message(command=0xA5, data=data)
    async with self._el406.batch():
      await self._el406._send_step_command(framed_command)
  ...
  async def dispense(
    ...
    Args:
      ...
      flow_rate: Dispense flow rate (1-11).
      offset_x: X offset in steps (-60 to +60).
      offset_y: Y offset in steps (-40 to +40).
      offset_z: Z offset in steps (1-210). None for plate-type default.
    ...
    """
    resolved_z = self._validate_dispense_params(
      self._el406.plate,
      volume,
      buffer=buffer,
      flow_rate=flow_rate,
    ...
    logger.info("Dispensing %.1f uL from buffer %s, flow rate %d", volume, buffer, flow_rate)
  ...
  async def wash(
    ...
    Args:
      ...
      buffer: Buffer valve selection (A, B, C, D).
      dispense_flow_rate: Flow rate for dispensing (1-9).
      dispense_x: Dispense X offset in steps (-60 to +60).
      dispense_y: Dispense Y offset in steps (-40 to +40).
      dispense_z: Z offset for dispense in 0.1mm units (1-210). None is
        plate-type-aware: 121 for 96-well, 120 for 384-well, etc.
      aspirate_travel_rate: Travel rate for aspiration (1-9).
      aspirate_z: Z offset for aspirate in 0.1mm units (1-210). None is
        plate-type-aware: 29 for 96-well, 22 for 384-well, etc.
      pre_dispense_flow_rate: Pre-dispense flow rate (3-11). Controls how fast the
        pre-dispense is delivered.
      aspirate_delay: Post-aspirate delay in seconds (0-65.535). Wire resolution: 1 ms.
      aspirate_x: Aspirate X offset in steps (-60 to +60).
      aspirate_y: Aspirate Y offset in steps (-40 to +40).
      final_aspirate: Enable final aspirate after last cycle. Encoded in header
        config flags byte [2].
      final_aspirate_z: Z offset for final aspirate (1-210). None inherits from
        aspirate_z. Independent from primary aspirate Z.
      final_aspirate_x: X offset for final aspirate (-60 to +60).
      final_aspirate_y: Y offset for final aspirate (-40 to +40).
      final_aspirate_delay: Post-aspirate delay for final aspirate in seconds
        (0-65.535). Wire resolution: 1 ms.
      pre_dispense_volume: Pre-dispense volume in uL/tube (0 to disable, 25-3000
        when enabled).
      vacuum_delay_volume: Vacuum delay volume in uL/well (0 to disable, 0-3000
        when enabled). Cell wash operations only.
      soak_duration: Soak duration in seconds (0 to disable, 0-3599).
      shake_duration: Shake duration in seconds (0 to disable, 0-3599).
      shake_intensity: Shake intensity ("Variable", "Slow", "Medium", "Fast").
      secondary_aspirate: Enable secondary aspirate for primary (between-cycle) aspirate.
      secondary_z: Z offset for secondary aspirate in 0.1mm units (1-210). None is
        plate-type-aware, same as aspirate_z default.
      ...
      final_secondary_z: Z offset for final secondary aspirate (1-210). None is
        plate-type-aware, same as aspirate_z default.
      ...
      bottom_wash: Enable bottom wash. Encoded in header[1].
      bottom_wash_volume: Bottom wash volume in uL (25-3000).
      bottom_wash_flow_rate: Bottom wash flow rate (3-11).
      pre_dispense_between_cycles_volume: Pre-dispense volume between wash cycles in
        uL (0 to disable, 25-3000 when enabled).
      pre_dispense_between_cycles_flow_rate: Flow rate for pre-dispense between
        cycles (3-11).
      wash_format: Wash format ("Plate" or "Sector"). Encoded at header[3]:
        Plate=0x00, Sector=0x01. 384-well plates typically use "Sector" for
        quadrant-based washing.
      sectors: List of quadrant numbers to wash (1-4). None means all 4. Example:
        ``sectors=[1, 2]`` washes quadrants 1 and 2. Only used when
        wash_format="Sector".
      move_home_first: Move carrier to home position before shake/soak. Same as in
        the standalone shake interface. Encoded at wire [87] (shake/soak section
        byte 0).
    ...
    data = self._build_wash_composite_command(
      ...
      dispense_x=dispense_x,
      dispense_y=dispense_y,
      dispense_z=resolved_dispense_z,
      aspirate_travel_rate=aspirate_travel_rate,
      aspirate_z=resolved_aspirate_z,
      pre_dispense_flow_rate=pre_dispense_flow_rate,
      ...
      secondary_y=secondary_y,
      final_secondary_aspirate=final_secondary_aspirate,
      final_secondary_z=resolved_final_secondary_z,
      final_secondary_x=final_secondary_x,
      final_secondary_y=final_secondary_y,
      bottom_wash=bottom_wash,
      bottom_wash_volume=bottom_wash_volume,
    ...
    async with self._el406.batch():
      await self._el406._send_step_command(framed_command, timeout=wash_timeout)
  ...
  async def prime(
    ...
    Args:
      ...
      buffer: Buffer valve selection (A, B, C, D).
      flow_rate: Flow rate (3-11).
      low_flow_volume: Low flow path volume in uL (5000-999000). Set to 0 to disable.
    ...
    prime_timeout = self._el406.timeout + submerge_duration + 30
    async with self._el406.batch():
      await self._el406._send_step_command(framed_command, timeout=prime_timeout)
  ...
  async def auto_clean(
    ...
    async with self._el406.batch():
      await self._el406._send_step_command(framed_command, timeout=auto_clean_timeout)
