"""The MultiFlo FX, a dispenser with syringes, two peristaltic pumps and a strip washer."""
...
PALETTE: tuple[StepType, ...] = (
  ...
  StepType.PERI_PRIME,
  StepType.PERI_PURGE,
  StepType.PERI_WASH_ASPIRATE,
  StepType.PERI_WASH_DISPENSE,
  StepType.STRIP_WASH,
  StepType.STRIP_ASPIRATE,
  StepType.STRIP_DISPENSE,
  StepType.STRIP_PRIME,
  StepType.SHAKE_SOAK,
)
"""Every step type this model can be built to run.

Which of them a particular instrument offers depends on both what is fitted and which firmware
variant is installed, and no instrument offers all of them at once: the peristaltic wash steps and
dispensing into individually chosen wells come from different firmware builds."""


class MultiFloFX:
  """A MultiFlo FX dispenser.

  The newest model in the family, and the only one whose peristaltic dispense head can be set from
  the host: opening a batch fits the head and the cassettes the checked protocol asked for.

  Its washing is done by a strip washer manifold rather than a plate wash manifold, so of the
  methods on :attr:`washer` only the strip ones run here; :meth:`get_available_steps` says which.
  ...
  Attributes:
    washer: The strip washer manifold.
    syringe_dispenser: The syringes.
    peristaltic_dispenser: The peristaltic pumps, including the wash manifolds.
  """
  ...
  def __init__(
    ...
    self.peristaltic_dispenser = PeristalticDispenser(self._runtime)
  ...
  def get_available_steps(self) -> list[StepType]:
    ...
    This model's firmware comes in variants offering different step types, and which variant is
    installed follows from what the instrument reports, so the palette is narrowed twice: by what
    is fitted, and by what the firmware knows.

    Returns:
      The step types, in the order this model offers them.
    """
    settings = self._runtime.settings
    fitted = set(available_step_types(settings))
    in_firmware = BASECODE_STEP_TYPES[basecode_for(settings)]
    return [step_type for step_type in PALETTE if step_type in fitted and step_type in in_firmware]
