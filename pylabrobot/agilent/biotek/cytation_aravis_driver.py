...
class CytationAravisDriver(BioTekBackend):
  ...
  - AravisCamera for image acquisition (Aravis/GenICam over USB3 Vision)
  ...
  def __init__(
    ...
    self.imaging_config = imaging_config or AravisImagingConfig(camera_serial_number=camera_serial)
  ...
  async def _load_filters(self) -> None:
    ...
    Uses the Cytation firmware filter code mapping.
    """
  ...
  async def set_focus(self, focal_position: FocalPosition) -> None:
    ...
    Uses a linear calibration for focus motor positioning.
    """
    if focal_position == "machine-auto":
      raise ValueError(
        "focal_position cannot be 'machine-auto'. Use PLR's Microscopy auto-focus instead."
      )
