...
def benchcel_4r_stacks(name_prefix: str = "benchcel_stacker") -> List[ResourceStack]:
  ...
  :class:`~pylabrobot.resources.resource_stack.ResourceStack` (a single-ended LIFO stack). Stack
  height is computed from each plate's ``stacking_z_height`` as plates are added, so -- unlike the
  previous fixed-site rack model -- no ``num_sites``/``site_pitch``/``site_height`` needs to be
  supplied.
