...
def calculate_benchcel_labware_settings(
  ...
  *,
  ...
  # BenchCel ``StackingThickness`` is the per-plate vertical pitch of a nested stack. Prefer an
  # explicit override, otherwise estimate it from height (``size_z - nesting_overlap``).
  # (A future ``Plate.stacking_z_height`` could drive this directly once available on this branch.)
