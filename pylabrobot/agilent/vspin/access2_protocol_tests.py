...
class Access2CommandTests(unittest.TestCase):
  ...
  def test_motion_captures(self):
    self.assertEqual(
      protocol.build_ftdi_frame(
        protocol.build_move_to_position(
  ...
  def test_load_motion_captures(self):
    commands = (
      (
        protocol.build_move_to_location(protocol.LOCATION_PICK, 3, 10),
        "1105000e00440b000100004040000020410200a5cb",
      ),
      (
        protocol.build_move_to_position(protocol.AXIS_GRIPPER, 5.68),
      ...
      (
        protocol.build_move_to_position(protocol.AXIS_GRIPPER, 0),
        "1105000a004607000100000000020015fd",
      ),
      (
        protocol.build_move_to_location(protocol.LOCATION_PARK, 3, 10),
  ...
  def test_unload_motion_captures(self):
    commands = (
      (
        protocol.build_move_to_location(protocol.LOCATION_BUCKET_1, 3, 10),
        "1105000e00440b000200004040000020410200dd31",
      ),
      (
        protocol.build_move_to_position(protocol.AXIS_GRIPPER, 5.69),
      ...
      (
        protocol.build_move_to_location(protocol.LOCATION_PARK, 0, 10),
...
class Access2ResponseTests(unittest.TestCase):
  ...
  def test_decode_full_status(self):
    ...
    gripper_position = status.gripper_position_mm
    ...
    self.assertEqual(status.y_position_mm, 100.5)
    self.assertEqual(status.z_position_mm, 20.25)
