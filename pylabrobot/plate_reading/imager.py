...
from pylabrobot.plate_reading.standard import ImagingMode, Exposure, FocalHeight, Gain
  ...
  async def capture(
    ...
    **backend_kwargs,
    ...
    if isinstance(well, tuple):
      ...
      if not well in self.plate.children:
        raise ValueError(f"Well {well} not in plate {self.plate}")
      idx = well.parent.index_of_item(well)
      row, column = divmod(idx, well.parent.num_items_x)
