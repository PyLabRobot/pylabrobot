...
PACKET_TYPE = {
    1: "MsgAscii", 2: "MsgTerminate", 3: "MsgBinary", 129: "RespReady", 130: "RespTerminate",
    131: "RespBinary", 132: "RespBusy", 133: "RespMessage", 134: "RespError", 135: "RespLog",
    136: "RespBinaryHeader", 137: "RespAsyncError",
}

# --- Payload Reader Helpers ---
def _read_bytes(payload: bytes, offset: int, length: int) -> bytes:
    if offset + length > len(payload):
        raise ValueError(f"Payload too short: need {length} bytes at offset {offset}, got {len(payload) - offset}")
    return payload[offset:offset+length]

def _read_u8(payload: bytes, offset: int) -> int: return struct.unpack('>B', _read_bytes(payload, offset, 1))[0]
def _read_u16(payload: bytes, offset: int) -> int: return struct.unpack('>H', _read_bytes(payload, offset, 2))[0]
def _read_u32(payload: bytes, offset: int) -> int: return struct.unpack('>I', _read_bytes(payload, offset, 4))[0]
def _read_string(payload: bytes, offset: int, length: int) -> str: return _read_bytes(payload, offset, length).decode('utf-8', errors='ignore')
...
class SparkPacket:
    ...
    def _parse_payload(self) -> Dict[str, Any]:
        try:
            if self.indicator == 129: return self._parse_resp_ready()
            if self.indicator == 130: return self._parse_resp_terminate()
            if self.indicator == 131: return self._parse_resp_binary(is_header=False)
            if self.indicator == 132: return self._parse_resp_busy()
            if self.indicator == 133: return self._parse_resp_message()
            if self.indicator == 134: return self._parse_resp_error(is_async=False)
            if self.indicator == 135: return self._parse_resp_log()
            if self.indicator == 136: return self._parse_resp_binary(is_header=True)
            if self.indicator == 137: return self._parse_resp_error(is_async=True)
    ...
    def _parse_resp_ready(self): return {"message": _read_string(self.payload_bytes, 0, len(self.payload_bytes)) if self.payload_bytes else None}
    def _parse_resp_terminate(self): return {"time": _read_u32(self.payload_bytes, 0) if len(self.payload_bytes) >= 4 else None}
    def _parse_resp_binary(self, is_header=False): return {"is_header": is_header, "data": self.payload_bytes}
    def _parse_resp_busy(self): return {"time": _read_u32(self.payload_bytes, 0) if len(self.payload_bytes) >= 4 else None}
    def _parse_resp_log(self): return {"message": _read_string(self.payload_bytes, 0, len(self.payload_bytes))}

    def _parse_resp_message(self):
        offset = 0
        number = _read_u16(self.payload_bytes, offset); offset += 2
    ...
    def _parse_resp_error(self, is_async=False):
        offset = 0
        timestamp = _read_u32(self.payload_bytes, offset); offset += 4
        number = _read_u16(self.payload_bytes, offset); offset += 2
        message_str = _read_string(self.payload_bytes, offset, len(self.payload_bytes) - offset)
        parts = message_str.split('|')
        return {"async": is_async, "timestamp": timestamp, "number": number, "format": parts[0], "args": parts[1:]}
...
class MeasurementBlock:
    ...
    def _ensure_buffer(self, num_bytes: int):
        while len(self.byte_buffer) < num_bytes:
            if not self.data_packets:
                raise ValueError(f"Incomplete data: Needed {num_bytes}, buffer has {len(self.byte_buffer)} for seq {self.seq_num}")
    ...
    def _parse_generic_payload(self, types: List[Dict[str, Any]]) -> Dict[str, Any]:
        parsed = {}
        for i, type_info in enumerate(types):
            ...
            if name == "x100U16TEMP": value /= 100.0
            if name in ["x10U16RWL", "x10U16MWL"]: value /= 10.0
    ...
    def parse(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.byte_buffer:
                ...
                logger.warning(f"Remaining buffer after parsing seq {self.seq_num}: {len(self.byte_buffer)} bytes")
    ...
    def _parse_nested_mult(self, result: Dict[str, Any], outer_mult_index: int, inner_mult_index: int):
    ...
    def _parse_linear(self, result: Dict[str, Any]):
        ...
        if len(self.byte_buffer) < total_size:
             logger.warning(f"Buffer size {len(self.byte_buffer)} less than expected {total_size} for linear parse in seq {self.seq_num}")
        ...
        for i, type_info in enumerate(self.header_types):
            ...
            if offset + size > len(self.byte_buffer):
                logger.warning(f"Payload too short for type {type_info['name']} at offset {offset} in linear parse seq {self.seq_num}")
...
class SparkParser:
    ...
    def process_all_sequences(self):
        ...
        for seq_num, packets_in_seq in self.sequences.items():
            ...
            if any(p.indicator == 136 for p in packets_in_seq): # Check for header
                ...
                logger.info(f"No header packet in sequence {seq_num}, skipping measurement stream processing.")
    ...
    def _process_measurement_stream(self, packets: List[SparkPacket]) -> List[Dict[str, Any]]:
        ...
        while header_idx < len(header_packets):
            ...
            header_type_names = [TDCL_DATA_TYPE_MAP.get(c, {}).get("name") for c in header_type_codes]
            ...
            if "U16MULT_H" in header_type_names:
                ...
                for i in range(num_headers):
                    if header_idx < len(header_packets):
                        ...
                        logger.warning(f"Expected {num_headers} headers, but only found {header_idx} in seq {seq_num}")
                        break
