...
from .spark_enums import SimulationState, ModuleType
...
class SystemControl(baseControl):
    ...
    async def get_version(self, hw_module: ModuleType=None, number=None, subcomponent=None):
    ...
    async def reset(self, hw_module: ModuleType=None, number=None, subcomponent=None):
    ...
    async def reset_to_bootloader(self, hw_module: ModuleType=None, number=None, subcomponent=None):
    ...
    async def define_simulation_data_generation_pattern(self, measurement_mode, data_pattern):
        ...
        return await self.send_command(f"SIMULATION MEASMODE={measurement_mode} GENERATE_DATA={data_pattern}")
