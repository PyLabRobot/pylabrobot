...
class measurement_control(baseControl):
  ...
  async def prepare_instrument(
    self, measure_reference=True, mode: Optional[MeasurementMode] = None, labels=None
  ):
  ...
  async def measure_position(
    self, x=None, y=None, z=None, mode: Optional[MeasurementMode] = None, labels=None
  ):
    return await self._measure_in(None, x=x, y=y, z=z, mode=mode, labels=labels)

  async def measure_current_position(self, mode: Optional[MeasurementMode] = None, labels=None):
