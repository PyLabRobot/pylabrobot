...
class ConfigControl(BaseControl):
  async def get_config_expected_modules(self) -> Optional[str]:
    """Gets the expected modules from configuration."""
    return await self.send_command("?CONFIG MODULE EXPECTED")

  async def set_config_expected_modules(self, module_details_list: List[Any]) -> Optional[str]:
  ...
  async def set_config_sap_instrument_serial_number(
    self,
    serial_number: str,
  ...
  async def set_config_module_serial_number(
    self,
    serial_number: str,
    module: ModuleType,
    module_number: int,
    ...
    command = (
      f"CONFIG INFO SAP_NR_MODULE={serial_number} MODULE={module.value} NUMBER={module_number}"
    )
    if sub_module:
      command += f" SUB={sub_module}"
    return await self.send_command(command)

  async def configure_home_direction(
    self,
    motor: str,
    value: str,
  ...
  async def configure_home_level_position(
    self,
    motor: str,
    value: int,
    hw_module: Optional[ModuleType] = None,
  ...
  async def configure_home_sensor_position(
    self,
    motor: str,
    value: int,
    hw_module: Optional[ModuleType] = None,
  ...
  async def configure_init_position(
    self,
    motor: str,
    value: int,
    hw_module: Optional[ModuleType] = None,
  ...
  async def configure_max_out_of_home(
    self,
    motor: str,
    value: int,
    hw_module: Optional[ModuleType] = None,
  ...
  async def get_home_level_position(
    self,
    motor: str,
    hw_module: Optional[ModuleType] = None,
    number: Optional[int] = None,
  ...
  async def get_home_sensor_position(
    self,
    motor: str,
    hw_module: Optional[ModuleType] = None,
    number: Optional[int] = None,
  ...
  async def get_config_limit_value(
    self,
    name: str,
    hw_module: Optional[ModuleType] = None,
    number: Optional[int] = None,
  ...
  async def set_config_limit_value(
    self,
    value: int,
    name: str,
    hw_module: Optional[ModuleType] = None,
  ...
  async def get_initializable_motors(
    self,
    hw_module: Optional[ModuleType] = None,
    number: Optional[int] = None,
    subcomponent: Optional[str] = None,
  ...
  async def set_offset(self, axis: ConfigAxis, offset: int) -> Optional[str]:
    """Sets the offset for a given axis (X, Y, Z)."""
    return await self.send_command(f"CONFIG OFFSET {axis.value}={offset}")

  async def get_offset(self, axis: ConfigAxis) -> Optional[str]:
    """Gets the offset for a given axis (X, Y, Z)."""
    return await self.send_command(f"?CONFIG OFFSET {axis.value}")

  async def set_x_offset(self, offset: int) -> Optional[str]:
    return await self.set_offset(ConfigAxis.X, offset)

  async def get_x_offset(self) -> Optional[str]:
    return await self.get_offset(ConfigAxis.X)

  async def set_y_offset(self, offset: int) -> Optional[str]:
    return await self.set_offset(ConfigAxis.Y, offset)

  async def get_y_offset(self) -> Optional[str]:
    return await self.get_offset(ConfigAxis.Y)

  async def set_z_offset(self, offset: int) -> Optional[str]:
    return await self.set_offset(ConfigAxis.Z, offset)

  async def get_z_offset(self) -> Optional[str]:
    return await self.get_offset(ConfigAxis.Z)

  async def set_mirror_offset(
    self, offset: int, module: ModuleType = ModuleType.FLUORESCENCE
  ) -> Optional[str]:
  ...
  async def get_mirror_offset(self, module: ModuleType = ModuleType.FLUORESCENCE) -> Optional[str]:
    """Gets the mirror offset for a given module."""
    return await self.send_command(f"?CONFIG OFFSET MIRROR1 MODULE={module.value}")

  async def _get_objective_config(
    self,
    index: int,
    param: str,
    hw_module: ModuleType = ModuleType.FLUORESCENCE_IMAGING,
    number: int = 1,
  ) -> Optional[str]:
  ...
  async def _set_objective_config(
    self,
    index: int,
    param: str,
    value: int,
  ...
  async def get_objective_magnification(self, index: int) -> Optional[str]:
    return await self._get_objective_config(index, "MAGNIFICATION")

  async def set_objective_magnification(self, index: int, value: int) -> Optional[str]:
    return await self._set_objective_config(index, "MAGNIFICATION", value)

  async def get_objective_autofocus_offset(self, index: int) -> Optional[str]:
    return await self._get_objective_config(index, "AF_OFFSET")

  async def set_objective_autofocus_offset(self, index: int, value: int) -> Optional[str]:
    return await self._set_objective_config(index, "AF_OFFSET", value)

  async def get_objective_z_offset(self, index: int) -> Optional[str]:
    return await self._get_objective_config(index, "Z_OFFSET")

  async def set_objective_z_offset(self, index: int, value: int) -> Optional[str]:
    return await self._set_objective_config(index, "Z_OFFSET", value)

  async def get_objective_brightfield_time(self, index: int) -> Optional[str]:
    return await self._get_objective_config(index, "BF_TIME")

  async def set_objective_brightfield_time(self, index: int, value: int) -> Optional[str]:
    return await self._set_objective_config(index, "BF_TIME", value)

  async def get_objective_roi_offset_x(
    self, hw_module: ModuleType = ModuleType.FLUORESCENCE_IMAGING, number: int = 1
  ) -> Optional[str]:
    return await self.send_command(
      f"?CONFIG OBJECTIVE AF_ROI_OFFSET_X MODULE={hw_module.value} NUMBER={number}"
    )

  async def set_objective_roi_offset_x(
    self, value: int, hw_module: ModuleType = ModuleType.FLUORESCENCE_IMAGING, number: int = 1
  ) -> Optional[str]:
    return await self.send_command(
      f"CONFIG OBJECTIVE AF_ROI_OFFSET_X={value} MODULE={hw_module.value} NUMBER={number}"
    )

  async def get_objective_roi_offset_y(
    self, hw_module: ModuleType = ModuleType.FLUORESCENCE_IMAGING, number: int = 1
  ) -> Optional[str]:
    return await self.send_command(
      f"?CONFIG OBJECTIVE AF_ROI_OFFSET_Y MODULE={hw_module.value} NUMBER={number}"
    )

  async def set_objective_roi_offset_y(
    self, value: int, hw_module: ModuleType = ModuleType.FLUORESCENCE_IMAGING, number: int = 1
  ) -> Optional[str]:
    return await self.send_command(
      f"CONFIG OBJECTIVE AF_ROI_OFFSET_Y={value} MODULE={hw_module.value} NUMBER={number}"
    )
