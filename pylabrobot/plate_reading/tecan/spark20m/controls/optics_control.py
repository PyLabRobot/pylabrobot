...
from .spark_enums import FilterType, FluorescenceCarrier, MirrorType, LightingType, LightingName, ModuleType
...
class OpticsControl(baseControl):
    ...
    async def get_emission_filter_type_list(self, carrier_name: FluorescenceCarrier=None):
    ...
    async def get_emission_filter_wavelength_list(self, carrier_name: FluorescenceCarrier=None, module: ModuleType=None, sub_module=None):
    ...
    async def get_emission_filter_bandwidth_list(self, carrier_name: FluorescenceCarrier=None):
    ...
    async def get_emission_filter_attenuation_list(self, carrier_name: FluorescenceCarrier=None):
    ...
    async def get_current_emission_filter(self, label=None, carrier: FluorescenceCarrier=None):
    ...
    async def set_emission_filter(self, filter_type: FilterType, wavelength=None, bandwidth=None, attenuation=None, label=None, carrier: FluorescenceCarrier=None):
        """Sets the emission filter."""
        command = f"EMISSION"
        if carrier: command += f" CARRIER={carrier.value}"
        if filter_type: command += f" TYPE={filter_type.value}"
        if wavelength is not None: command += f" WAVELENGTH={wavelength}"
        if bandwidth is not None: command += f" BANDWIDTH={bandwidth}"
        if attenuation is not None: command += f" ATTENUATION={attenuation}"
        if label is not None: command += f" LABEL={label}"
        return await self.send_command(command)
    ...
    async def get_empty_position_wavelength_limit_lower(self, hw_module: ModuleType=None, number=None):
    ...
    async def get_empty_position_wavelength_limit_upper(self, hw_module: ModuleType=None, number=None):
    ...
    async def set_empty_position_wavelength_limits(self, lower, upper, hw_module: ModuleType=None, number=None):
    ...
    async def get_excitation_filter_type_list(self, carrier_name: FluorescenceCarrier=None):
    ...
    async def get_excitation_filter_wavelength_list(self, carrier_name: FluorescenceCarrier=None, module: ModuleType=None, sub_module=None):
    ...
    async def get_excitation_filter_bandwidth_list(self, carrier_name: FluorescenceCarrier=None):
    ...
    async def get_excitation_filter_attenuation_list(self, carrier_name: FluorescenceCarrier=None):
    ...
    async def get_current_excitation_filter(self, label=None, carrier: FluorescenceCarrier=None):
    ...
    async def set_excitation_filter(self, filter_type: FilterType, wavelength=None, bandwidth=None, attenuation=None, label=None, carrier: FluorescenceCarrier=None):
        """Sets the excitation filter."""
        command = f"EXCITATION"
        if carrier: command += f" CARRIER={carrier.value}"
        if filter_type: command += f" TYPE={filter_type.value}"
        if wavelength is not None: command += f" WAVELENGTH={wavelength}"
        if bandwidth is not None: command += f" BANDWIDTH={bandwidth}"
    ...
    async def set_defined_filter_wavelength(self, name, position, wavelength):
        ...
        return await self.send_command(f"DEFINE FILTER NAME={name} POSITION={position} WAVELENGTH={wavelength}")
    ...
    async def set_defined_filter_bandwidth(self, name, position, bandwidth):
        ...
        return await self.send_command(f"DEFINE FILTER NAME={name} POSITION={position} BANDWIDTH={bandwidth}")
    ...
    async def set_defined_filter_type(self, name, position, filter_type):
        ...
        return await self.send_command(f"DEFINE FILTER NAME={name} POSITION={position} TYPE={filter_type}")
    ...
    async def set_defined_filter_flash_counter(self, name, position, flash_counter):
        ...
        return await self.send_command(f"DEFINE FILTER NAME={name} POSITION={position} FLASH_COUNTER={flash_counter}")
    ...
    async def set_defined_filter_name(self, name, position, filter_name):
        ...
        return await self.send_command(f"DEFINE FILTER NAME={name} POSITION={position} DESCRIPTION={filter_name}")
    ...
    async def set_signal_gain(self, gain, label=None, wavelength=None, channel=None):
        ...
        if wavelength is not None: command += f" WAVELENGTH={wavelength}"
        if channel is not None: command += f" CHANNEL={channel}"
        return await self.send_command(command)
    ...
    async def set_reference_gain(self, gain, label=None, wavelength=None, carrier: FluorescenceCarrier=None):
    ...
    async def set_laser_power(self, state: LaserPowerState, hw_module: ModuleType=None, number=None, subcomponent=None):
    ...
    async def set_intensity_fim(self, name, light_type, intensity, hw_module: ModuleType=None, module_number=None):
        """Sets the intensity for FIM lighting."""
        command = f"LIGHTING TYPE={light_type.upper()} NAME={name.upper()} INTENSITY={intensity}"
        if hw_module: command += f" MODULE={hw_module.value}"
        if module_number is not None: command += f" NUMBER={module_number}"
        return await self.send_command(command)

    async def set_state_fim(self, name, light_type, is_on: LightingState):
        """Sets the state for FIM lighting."""
        return await self.send_command(f"LIGHTING TYPE={light_type.upper()} NAME={name.upper()} STATE={is_on.value}")
    ...
    async def get_lighting_details_fim(self, name, light_type):
        ...
        return await self.send_command(f"#LIGHTING TYPE={light_type.upper()} NAME={name.upper()} INTENSITY")
    ...
    async def set_mirror(self, mirror_type: MirrorType, mirror_name=None, carrier: MirrorCarrier=None, label=None):
        """Sets the mirror type and name."""
        command = f"MIRROR TYPE={mirror_type.value}"
        if mirror_name: command += f" NAME={mirror_name}"
        if carrier: command += f" CARRIER={carrier.value}"
        if label is not None: command += f" LABEL={label}"
        return await self.send_command(command)
    ...
    async def get_mirror_types(self, carrier: MirrorCarrier=None):
    ...
    async def get_mirror_names(self, carrier: MirrorCarrier=None):
    ...
    async def _get_mirror_wavelengths(self, option, carrier: MirrorCarrier):
        command = "#MIRROR"
        if carrier: command += f" CARRIER={carrier.value}"
        command += f" {option}"
        response = await self.send_command(command)
    ...
    async def define_mirror_name(self, name, position, carrier: MirrorCarrier=None):
        """Defines the name for a mirror at a specific position."""
        command = f"DEFINE MIRROR"
        if carrier: command += f" CARRIER={carrier.value}"
        command += f" POSITION={position} NAME={name} {self._create_mirror_settings_target()}"
        return await self.send_command(command)

    async def define_mirror_type(self, mirror_type: MirrorType, position, carrier: MirrorCarrier=None):
        """Defines the type for a mirror at a specific position."""
        command = f"DEFINE MIRROR"
        if carrier: command += f" CARRIER={carrier.value}"
        command += f" POSITION={position} TYPE={mirror_type.value}{self._create_mirror_settings_target()}"
        return await self.send_command(command)

    async def define_mirror_excitation_start(self, ex_start, position, carrier: MirrorCarrier=None):
        """Defines the excitation start wavelength for a mirror."""
        command = f"DEFINE MIRROR"
        if carrier: command += f" CARRIER={carrier.value}"
        command += f" POSITION={position} EXCITATION_START={ex_start}{self._create_mirror_settings_target()}"
        return await self.send_command(command)

    async def define_mirror_excitation_end(self, ex_end, position, carrier: MirrorCarrier=None):
        """Defines the excitation end wavelength for a mirror."""
        command = f"DEFINE MIRROR"
        if carrier: command += f" CARRIER={carrier.value}"
        command += f" POSITION={position} EXCITATION_END={ex_end}{self._create_mirror_settings_target()}"
        return await self.send_command(command)

    async def define_mirror_emission_start(self, em_start, position, carrier: MirrorCarrier=None):
        """Defines the emission start wavelength for a mirror."""
        command = f"DEFINE MIRROR"
        if carrier: command += f" CARRIER={carrier.value}"
        command += f" POSITION={position} EMISSION_START={em_start}{self._create_mirror_settings_target()}"
        return await self.send_command(command)

    async def define_mirror_emission_end(self, em_end, position, carrier: MirrorCarrier=None):
    ...
    async def get_defineable_mirror_positions(self, carrier: MirrorCarrier=None):
    ...
    async def get_objective_details(self, objective_type, hw_module: ModuleType=None, module_number=None):
    ...
    async def get_mtp_allowed_area(self, objective_type, mtp_area_type):
        ...
        return await self.send_command(f"#OBJECTIVE TYPE={objective_type.upper()} RANGE={mtp_area_type.upper()}")
