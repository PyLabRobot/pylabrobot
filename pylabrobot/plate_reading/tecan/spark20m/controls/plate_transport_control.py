...
class PlateControl(BaseControl):
  ...
  async def stacker_stack_plate(self, column_type, plate_height, skirt_height):
    ...
    return await self.send_command(
      f"STACKER STACK COLUMN={column_type.upper()} PLATEHEIGHT={plate_height} SKIRTHEIGHT={skirt_height}"
    )

  async def stacker_get_plate(
    self, column_type, plate_height, skirt_height, color: PlateColor = PlateColor.NO
  ):
    """Commands the stacker to get a plate."""
    return await self.send_command(
      f"STACKER GET COLUMN={column_type.upper()} PLATEHEIGHT={plate_height} SKIRTHEIGHT={skirt_height} COLOUR={color.value}"
    )
