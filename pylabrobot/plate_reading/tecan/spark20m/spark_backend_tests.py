...
# Mock dependencies that might require hardware or external libs
...
class TestSparkBackend(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        ...
        self.reader_patcher = patch('pylabrobot.plate_reading.tecan.spark20m.spark_backend.SparkReaderAsync')
        ...
        # Patch Processors
        self.abs_proc_patcher = patch('pylabrobot.plate_reading.tecan.spark20m.spark_backend.AbsorbanceProcessor')
        self.MockAbsProcClass = self.abs_proc_patcher.start()
        self.mock_abs_proc = self.MockAbsProcClass.return_value

        self.fluo_proc_patcher = patch('pylabrobot.plate_reading.tecan.spark20m.spark_backend.FluorescenceProcessor')
