...
from pylabrobot.plate_reading.tecan.spark20m.spark_processor import AbsorbanceProcessor, FluorescenceProcessor
...
class TestAbsorbanceProcessor(unittest.TestCase):
            ...
            "SEQ_REF": [
                {
                    "type": "grouped",
                    "blocks": [
                        {"rd_md_pairs": dark_pairs},
                        {"rd_md_pairs": ref_pairs}
                    ]
                }
            ],
            "SEQ_MEAS": [
    ...
    def test_process_missing_reference(self):
        ...
        parsed_data = {
            "SEQ_MEAS": [
                {"type": "standalone", "block": {"measurements": []}}
            ]
        }
        with patch.object(self.processor, '_parse_raw_data', return_value=parsed_data):
            results = self.processor.process([])
...
class TestFluorescenceProcessor(unittest.TestCase):
    ...
    def test_process_success(self):
        ...
        meas_block = {
            ...
            "measurements": [
                {
                    "inner_loops": [{"U16MD_1": 20000, "U16RD_0": 30000}]
                }
            ]
        }
            ...
            "SEQ_CAL": [
                {
                    "type": "grouped",
                    "count": 2,
                    "blocks": [dark_block, bright_block]
                }
            ],
            "SEQ_MEAS": [
                {
    ...
    def test_process_missing_calibration(self):
        parsed_data = {
            "SEQ_MEAS": [
                {
                    "type": "standalone",
                    "block": {"structure_type": "nested_mult", "measurements": []}
                }
            ]
        }
        with patch.object(self.processor, '_parse_raw_data', return_value=parsed_data):
