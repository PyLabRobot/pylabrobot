...
from .controls.plate_transport_control import plateControl, MovementSpeed, PlatePosition
from .controls.measurement_control import measurement_control, MeasurementMode, ScanDirection
from .controls.optics_control import OpticsControl, FilterType, FluorescenceCarrier, MirrorType
from .controls.system_control import SystemControl
from .controls.sensor_control import SensorControl, InstrumentMessageType
...
class SparkBackend(PlateReaderBackend):
    ...
    def __init__(self, vid: int = 0x0c47) -> None:
        ...
        # Initialize .controls
        self.config = AutoReaderProxy(ConfigControl(self.reader), self.reader, SparkDevice.PLATE_TRANSPORT)
        self.plate = AutoReaderProxy(plateControl(self.reader), self.reader, SparkDevice.PLATE_TRANSPORT)
        self.measure = AutoReaderProxy(measurement_control(self.reader), self.reader, SparkDevice.PLATE_TRANSPORT)
        self.optics = AutoReaderProxy(OpticsControl(self.reader), self.reader, SparkDevice.PLATE_TRANSPORT)
        self.system = AutoReaderProxy(SystemControl(self.reader), self.reader, SparkDevice.PLATE_TRANSPORT)
        self.sensors = AutoReaderProxy(SensorControl(self.reader), self.reader, SparkDevice.PLATE_TRANSPORT)
    ...
    async def scan_plate_range(self, plate:Plate, wells: Optional[List[Well]],z:float=9150):
        ...
        for min_row, min_col, max_row, max_col in rects:
            for row_idx in range(min_row, max_row + 1):
                ...
                await self.measure.measure_range_in_x_pointwise(
                    start_x,
                    end_x,
                    y_pos,
                    z,
    ...
    async def read_fluorescence(
        ...
        await self.optics.set_emission_filter(FilterType.BANDPASS, wavelength=em_wavelength, bandwidth=bandwidth, carrier=FluorescenceCarrier.MONOCHROMATOR_EMISSION)
        await self.optics.set_excitation_filter(FilterType.BANDPASS, wavelength=ex_wavelength, bandwidth=bandwidth, carrier=FluorescenceCarrier.MONOCHROMATOR_EXCITATION)
