...
- Add `RoboticArm` as a machine subclass with `WaveshareRoam` as a backend. Details for how to set up a RPI server to control this are included in a README.
...
- Removed `keep_alive_thread` from `agrowdosepump.py`. This is not necessary and can complicate error handling and messages. Users are expected to implement their own safety precautions.
