...
- Agilent BenchCel 4R microplate handler (`pylabrobot.agilent.benchcel.BenchCel4R`) with four tracked `ResourceStack` objects; its driver speaks the reverse-engineered binary TCP/7612 protocol and implements `downstack`/`upstack`, calculation-based labware helpers, and a `set_labware(...)` push (`0x7d`/`0x9f`) (#1113).
