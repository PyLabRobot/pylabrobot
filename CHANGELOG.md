...
- `StandingTipRack`: a tip rack that stands on its own rather than sinking into a holder, with an optional `stacking_z_height` for racks that nest in a z-growing `ResourceStack`; the height round-trips through `serialize`.
- Hamilton nested tip racks (NTR) `hamilton_96_tiprack_10uL_NTR` and `hamilton_96_tiprack_300uL_NTR`, built with `hamilton_96_tiprack_ntr` like `hamilton_96_tiprack_50uL_NTR`. Their tip spots reproduce the firmware positions Venus sends on `hamilton_tip_carrier_L5_ntr_a00`.
- `hamilton_mfx_resource_holder_ntr4`: the MFX module for a stack of nested tip racks (191425). The racks' tip spots reproduce the firmware positions Venus sends on its NTR4 site.

### Changed

- `hamilton_96_tiprack_50uL_NTR` is a `StandingTipRack` on an SLAS footprint (127.76 x 85.48, 55 tall) with its tips in the SLAS positions, and its tip spots where the collars rest, 55 mm above the rack's bottom (was 13.5), so a tip is placed like one in an embedded rack. `hamilton_tip_carrier_L5_ntr_a00` stands those footprints centred on its sites, and the Vantage discard takes the tip end from these spots as it does for embedded racks.
