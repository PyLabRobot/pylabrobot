...
Microplates are modelled by the :class:`~pylabrobot.resources.plate.Plate` class consisting of equally spaced wells. Wells are children of the ``Plate`` and are modelled by the :class:`~pylabrobot.resources.well.Well` class. The relative positioning of a ``Well`` is what determines its location. ``Plate`` is a subclass of :class:`~pylabrobot.resources.itemized_resource.ItemizedResource`, allowing convenient integer and string indexing.
